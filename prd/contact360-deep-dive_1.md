...
def discover_pattern(known_emails: List[Tuple[str, str, str]]) -> str | None:
    ...
    # Only return pattern if it matches >60% of samples
    confidence = count / len(known_emails)
    if confidence >= 0.6:
        return most_common
...
```
# ── Caching ───────────────────────────────────────────────────────────────────
contact:{contact_id}                   STRING  TTL: 5min   Full contact JSON
search:{org_id}:{query_hash}           STRING  TTL: 60s    Search results JSON
company:{company_id}                   STRING  TTL: 10min  Company profile JSON
user_session:{user_id}                 HASH    TTL: 24h    Session metadata

# ── Rate Limiting (Token Bucket) ─────────────────────────────────────────────
rate_limit:hunter:{org_id}             STRING  TTL: 60s    Request count (max 100/min)
rate_limit:zerobounce:{org_id}         STRING  TTL: 60s    Request count (max 200/min)
rate_limit:millionverif:{org_id}       STRING  TTL: 60s    Request count (max 500/min)
rate_limit:twilio:{org_id}             STRING  TTL: 60s    SMS count (TRAI: max 30/min)
rate_limit:ai_chat:{user_id}           STRING  TTL: 60s    AI query count (max 20/min)

# ── Campaign Stats (write-through buffer) ────────────────────────────────────
campaign_stats:{campaign_id}           HASH    TTL: none   {sent, delivered, opened, clicked, replied}

# ── Job Queues (Bull uses Redis internally) ───────────────────────────────────
bull:campaign-dispatch:waiting         LIST    Job IDs waiting to process
bull:campaign-dispatch:active          SET     Job IDs currently processing
bull:campaign-dispatch:completed       ZSET    Completed job IDs (score = timestamp)
bull:campaign-dispatch:failed          ZSET    Failed job IDs

# ── AI Workflow State (for pending approvals) ─────────────────────────────────
workflow:pending:{workflow_id}         STRING  TTL: 24h    Serialized AgentState
workflow:approved:{workflow_id}        STRING  TTL: 5min   Approval status flag

# ── Distributed Locks ─────────────────────────────────────────────────────────
lock:enrich:{contact_id}               STRING  TTL: 30s    Prevent duplicate enrichment
lock:campaign_launch:{campaign_id}     STRING  TTL: 60s    Prevent double-launch
lock:flush_stats:{campaign_id}         STRING  TTL: 10s    Prevent concurrent stat flushes

# ── Pub/Sub Channels ─────────────────────────────────────────────────────────
channel:org:{org_id}:notifications     PUBSUB  Real-time notifications
channel:workflow:{workflow_id}:updates PUBSUB  Workflow status updates
```
...
interface ContactCreatedEvent {
  ...
  last_name:   string;
  domain?:     string;
  source:      "extension" | "import" | "manual" | "ai" | "api";
  timestamp:   string;  // ISO 8601
}

// Topic: contact.enriched
interface ContactEnrichedEvent {
  event:       "contact.email_enriched" | "contact.phone_enriched";
  contact_id:  string;
  org_id:      string;
  email?:      string;
  phone?:      string;
...
interface CampaignLaunchedEvent {
...
interface CampaignEngagementEvent {
  event:        "campaign.engagement";
  email_log_id: string;
  campaign_id:  string;
  contact_id:   string;
  org_id:       string;
  engagement:   "opened" | "clicked" | "replied" | "unsubscribed";
  metadata?:    { url?: string; user_agent?: string };
  timestamp:    string;
}

// Topic: ai.workflow.completed
interface AIWorkflowCompletedEvent {
...
## 10. End-to-End Flow: Lead Conversion

```
User (Slack)                AI Service               CRM Service             Email Service           Analytics
     │                          │                         │                       │                       │
     │── "Score this lead: ──►  │                         │                       │                       │
     │    john@acme.com"         │                         │                       │                       │
     │                          │── classify_intent() ──► │                       │                       │
     ...
     │                          │                          │                       │                       │
     │                          │── retrieve_context() ──►│                       │                       │
     │                          │   pgvector search        │── SELECT contacts ──► │                       │
     │                          │   full-text search       │   WHERE email=... ─── │                       │
     │                          │   ◄─ contact record ─────│                       │                       │
     │                          │   ◄─ deal history ───────│                       │                       │
     │                          │   ◄─ email engagement ───│──────────────────────►│                       │
     │                          │                          │                       │                       │
...
Total latency: ~1.2-2.5 seconds (cold) / ~300ms (warm cache)
```

---

*Document generated: April 14, 2026 | Contact360 v1.0 | Koushik Chandra Saha*
