...
- 📌 Planned: Update **`emailapis_endpoint_era_matrix.json`** — **Service task slices** in `2.6.P` patch files (scope from former `emailapis-email-system-task-pack.md`).  
- 📌 Planned: Publish **status enum** table shared by gateway, app, Lambdas.

### Service

- 📌 Planned: **Parity tests** same payload → same normalized output (Python vs Go).  
