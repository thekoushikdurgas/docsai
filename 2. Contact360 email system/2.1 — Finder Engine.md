...
- 📌 Planned: Freeze **finder** GraphQL input/output and enum for pattern sources — **Service task slices** in `2.1.P` patch files (scope from former `appointment360-email-system-task-pack.md`).  
- 📌 Planned: Document **emailapigo** vs **emailapis** routing rules — **Service task slices** in `2.1.P` patch files (scope from former `emailapis-email-system-task-pack.md`).

### Service

- 📌 Planned: Implement or verify **10-pattern generation** and **historical log** behavior per roadmap.  
- 📌 Planned: Harden **provider fallback** when primary provider fails (no silent empty result).

### Surface

- 📌 Planned: **App:** `EmailFinderSingle.tsx` + `useEmailFinderSingle.ts` aligned to contract.  
- 📌 Planned: Loading, empty, and error states explicit.

### Data

- 📌 Planned: **`email_finder_cache`** mutation rules and TTL or invalidation policy documented.  
- 📌 Planned: **`email_patterns`** lineage for user-added patterns.

### Ops

- 📌 Planned: Latency dashboard: p50/p95 finder (roadmap KPI).  
- 📌 Planned: Alert on finder error rate spike.
