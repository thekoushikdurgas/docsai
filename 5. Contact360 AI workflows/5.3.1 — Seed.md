...
- 📌 Planned: **Tenant isolation:** Assert every AI-originated query path resolves through tenant-scoped API keys and cannot bypass filter predicates.
- 📌 Planned: Lock `SendMessageInput.model` contract: accepted values and mapping documented in `17_AI_CHATS_MODULE.md`.
- 📌 Planned: Define prompt-safe output contract (no sensitive raw SMTP payload).
