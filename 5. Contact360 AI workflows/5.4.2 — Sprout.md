...
## Tasks
### Contract
- ✅ Completed: 📌 Planned: **AI-facing field whitelist:** Document stable fields (names + types) for contacts/companies that may be embedded in prompts or tool results; exclude sensitive or volatile columns by default.
- ✅ Completed: 📌 Planned: Lock full REST API contract: all `/api/v1/ai-chats/` and `/api/v1/ai/` paths.
- ✅ Completed: 📌 Planned: Document `POST /api/v1/ai-chats/{id}/message/stream` SSE event format: `data: <token>\n\n`, `data: [DONE]\n\n`.
- ✅ Completed: 📌 Planned: Define **four AI artifact classes** (stable enum):
