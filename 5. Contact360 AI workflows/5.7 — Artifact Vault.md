...
- 📌 Planned: Document retrieval semantics for internal tools ([`s3storage-ui-bindings.md`](../frontend/s3storage-ui-bindings.md)).

### Data

- 📌 Planned: Link `source_file_id` → `run_id` → `output_artifact_id` in metadata conventions.

### Ops

- 📌 Planned: Audit trail for sensitive artifact reads.
- 📌 Planned: Lifecycle transitions (Glacier, deletion) documented.
