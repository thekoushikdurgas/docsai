...
- 📌 Planned: Minimum SN contact eligibility for AI: title, `data_quality_score` threshold, `about` presence (**Service task slices** in `5.5.P` patch files (scope from former `salesnavigator-ai-workflows-task-pack.md`)).
...
- 📌 Planned: **app**: `DataQualityBar`, “AI-ready” when score ≥ policy threshold; company summary for SN imports.
- 📌 Planned: **extension**: Profiles feed Connectra (indirect AI dependency).

### Data

- 📌 Planned: Confirm field coverage in ES mappings and PG hydration.

### Ops

- 📌 Planned: Monitor AI errors correlated with low `data_quality_score`.
- 📌 Planned: Alert on high share of score &lt; 30 from SN sessions.
