...
- 📌 Planned: VQL operator allow-list for AI-generated filters (coordinate with `5.2`).
...
- 📌 Planned: **root/admin**: Storytelling / governance copy referencing data accuracy (**Service task slices** in `5.10.P` patch files (scope from former `connectra-ai-task-pack.md`)).
- 📌 Planned: **app**: Any “preview” of AI-bound payloads uses whitelist fields only.

### Data

- 📌 Planned: Lineage: enrichment artifact pointers on entities where applicable.
- 📌 Planned: ES mappings support fields AI code depends on (e.g. quality scores).

### Ops

- 📌 Planned: AI query regression pack: representative VQL from `parse-filters`.
- 📌 Planned: Cost-impact analysis for AI-driven query volume.
