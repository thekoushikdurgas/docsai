...
- Image build status:
  - `sync.server`, `job.server`, `s3storage.server`, `ai.server`, `log.server`, `extension.server`, `email campaign` => build success.
- Runtime status:
  ...
  - `job.server` container exits immediately (`HTTP 000`, `2.240538s`) with `.env` not found in `/app`.
...
- [ ] Add `.env.example` copy into final image or make `.env` optional in `job.server` runtime.
