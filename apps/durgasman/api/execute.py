...
def execute_request(request):
    ...
    try:
        ...
        environment_vars = {}
        try:
            environment_id = data.get('environment_id') or data.get('environment_name')
            if environment_id:
                env = environment_storage.get_environment(environment_id)
                if env and env.get('user') == user_uuid:
                    variables = env.get('variables_list', []) or env.get('variables', [])
                    environment_vars = {
        ...
        # Convert headers list to dict for storage
