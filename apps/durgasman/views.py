...
def dashboard(request):
    ...
    for c in collections:
        c['requests_count'] = len(c.get('requests', []))
        c['id'] = c.get('collection_id') or c.get('id')
    ...
    # Load Postman index via discovery (scans root + folders) and media files
    postman_index = {}
    try:
        ...
        try:
            ...
            for e in postman_index.get('environments', []):
                if not e.get('relative_path') and e.get('file_name'):
                    e['relative_path'] = f"postman/{e['file_name']}"
            if 'folder_collections' not in postman_index:
                postman_index['folder_collections'] = []
        except Exception:
            postman_index = {'collections': [], 'environments': [], 'folder_collections': [], 'configurations': []}

    postman_media_files = []

    try:
        file_manager = MediaFileManagerService()
        raw_files = file_manager.scan_media_directory('postman')
        postman_media_files = []
        for f in raw_files:
            ...
            if modified is not None:
                try:
                    if isinstance(modified, datetime):
                        f_copy['modified_display'] = modified.strftime('%b %d, %Y %H:%M')
                    else:
                        f_copy['modified_display'] = datetime.fromtimestamp(modified).strftime('%b %d, %Y %H:%M')
                except (TypeError, OSError, ValueError):
                    f_copy['modified_display'] = '-'
            else:
                f_copy['modified_display'] = '-'
            postman_media_files.append(f_copy)
    except Exception:
        postman_media_files = []

    # Add index.json to media files if it exists
    postman_dir = get_postman_dir()
    index_path = postman_dir / 'index.json'
    if index_path.exists():
        try:
            st = index_path.stat()
            postman_media_files.insert(0, {
                'file_path': str(index_path),
                'relative_path': 'postman/index.json',
                'name': 'index.json',
                'size': st.st_size,
                'modified': st.st_mtime,
                'modified_display': datetime.fromtimestamp(st.st_mtime).strftime('%b %d, %Y %H:%M'),
                'resource_type': 'postman',
                'subdirectory': 'postman',
            })
        except OSError:
            pass
    ...
    context = {
        ...
        'postman_media_files': postman_media_files,
    }
...
def collection_detail(request, collection_id):
    ...
    for req in requests:
        if 'id' not in req and 'request_id' in req:
            req['id'] = req['request_id']
        elif 'request_id' not in req and 'id' in req:
            req['request_id'] = req['id']

    # Group requests by folder (e.g. "System/Root Metadata" -> folder "System", display "Root Metadata")
    folders = {}
    for req in requests:
        name = req.get('name', '')
        parts = name.split('/', 1)
    ...
    # Sort folders: empty string (root) first, then alphabetical
    folders_sorted = [(k, v) for k, v in sorted(folders.items(), key=lambda x: (x[0] != '', x[0].lower()))]
...
def _validate_durgasman_import_path(file_path: str, allow_folder: bool = False):
    """Validate file path for Durgasman import. Returns (relative_path, full_path) or (None, None)."""
    media_root = get_media_root()
    fp = unquote(file_path).replace("\\", "/").lstrip("/")
    # Strip leading media/ if present (Media Manager may pass either format)
    if fp.lower().startswith("media/"):
        fp = fp[6:].lstrip("/")
    # Security: reject path traversal
    if ".." in fp or fp.startswith("/"):
        return None, None
    full = (media_root / fp).resolve()
    # Ensure path is under media root
    try:
    ...
    if not full.exists():
        return None, None
    if allow_folder and full.is_dir():
        return fp, full
    if not full.is_file():
        return None, None
    return fp, full
...
def import_view(request):
    ...
    # Supported import types: postman, endpoints, environment, requestly_folder
    if import_type not in ('postman', 'endpoints', 'environment', 'requestly_folder'):
        messages.info(
            request,
            f'"{import_type}" files cannot be imported into Durgasman. '
            'Use Postman collections, environments, Requestly folders, or Endpoint JSON files.'
        )
        return redirect('durgasman:dashboard')

    allow_folder = import_type == 'requestly_folder'
    rel_path, full_path = _validate_durgasman_import_path(file_path, allow_folder=allow_folder)
    if not rel_path or not full_path:
        messages.error(request, f'File or folder not found: {file_path}')
        return redirect('durgasman:dashboard')

    try:
        if import_type == 'requestly_folder':
            from .services.postman_importer import import_requestly_folder
            collection = import_requestly_folder(str(full_path), user_uuid)
            messages.success(request, f'Successfully imported folder collection: {collection.get("name")}')
            ...
            collection = import_postman_collection(str(full_path), user_uuid)
            ...
            collection = import_endpoint_json(str(full_path), user_uuid)
            messages.success(request, f'Successfully imported endpoint: {collection.get("name")}')
            return redirect('durgasman:collection_detail', collection_id=collection.get('collection_id') or collection.get('id'))
        elif import_type == 'environment':
            from .services.postman_importer import import_postman_environment
            environment = import_postman_environment(str(full_path), user_uuid)
            messages.success(request, f'Successfully imported environment: {environment.get("name")}')
            return redirect('durgasman:dashboard')
        else:
...
def api_mocks(request):
    ...
    for collection in collections_result.get('items', []):
        mock_endpoints = collection.get('mocks', [])
