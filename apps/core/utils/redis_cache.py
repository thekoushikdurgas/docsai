...
class RedisCacheManager:
    ...
    def __init__(self, cache_version: str = CACHE_VERSION):
        ...
        if self.is_redis_enabled:
            logger.debug(
                "RedisCacheManager initialized with Redis backend (version: %s)",
                cache_version,
            )
        else:
            logger.debug(
                "RedisCacheManager initialized with Django default cache backend (LocMemCache, version: %s)",
                cache_version,
            )
    ...
    def set(
        ...
        try:
            ...
            logger.debug(
                "Cache set: %s (TTL: %ds, type: %s)",
                full_key,
                timeout,
                data_type or "default",
            )
            return True
        except Exception as e:
    ...
    def invalidate_by_data_type(
        self, data_type: str, namespace: Optional[str] = None
    ) -> int:
        """
        ...
        logger.debug(
            "Invalidate by data type not fully implemented yet (data_type: %s)",
            data_type,
        )
        return 0
    ...
    def delete_pattern(self, pattern: str, namespace: Optional[str] = None) -> int:
        ...
        if not self._is_redis_backend():
            ...
            logger.debug(
                "delete_pattern only works with Redis backend (using %s), skipping pattern deletion",
        ...
        try:
            ...
            if hasattr(self.cache_backend, "client"):
                ...
                if keys:
                    ...
                    logger.debug(
                        "Deleted %d keys matching pattern %s", deleted, full_pattern
                    )
                    return deleted
            return 0
        except Exception as e:
            logger.warning(
                "Cache delete_pattern failed for pattern %s: %s", full_pattern, e
            )
            return 0
    ...
    def _build_key(
        self, key: str, namespace: Optional[str] = None, include_version: bool = True
    ) -> str:
        """
    ...
    def decorator(func: Callable[..., T]) -> Callable[..., T]:
        ...
        def wrapper(*args: Any, **kwargs: Any) -> T:
            ...
            if cached_value is not None:
                logger.debug(
                    "Cache hit for function %s with key %s", func.__name__, cache_key
                )
                return cached_value
            ...
            if condition is None or condition(result, *args, **kwargs):
                cache_manager.set(
                    cache_key, result, timeout=timeout, namespace=namespace
                )
                logger.debug(
                    "Cached result for function %s with key %s",
                    func.__name__,
                    cache_key,
                )
    ...
    def decorator(func: Callable[..., T]) -> Callable[..., T]:
        ...
        def wrapper(*args: Any, **kwargs: Any) -> T:
            ...
            cache_key = cache_manager.generate_key(
                CACHE_PREFIX_ENDPOINTS, endpoint_id=endpoint_id
            )
...
def invalidate_endpoint_cache(endpoint_id: Optional[str] = None) -> int:
    ...
    """
    if endpoint_id:
        pattern = cache_manager.generate_key(
            CACHE_PREFIX_ENDPOINTS, endpoint_id=endpoint_id
        )
        return cache_manager.delete_pattern(f"{pattern}*")
    else:
...
def invalidate_relationship_cache(relationship_id: Optional[str] = None) -> int:
    ...
    if relationship_id:
        pattern = cache_manager.generate_key(
            CACHE_PREFIX_RELATIONSHIPS, relationship_id=relationship_id
        )
        return cache_manager.delete_pattern(f"{pattern}*")
    else:
...
def invalidate_graphql_cache(query: Optional[str] = None) -> int:
    ...
    if query:
        ...
        pattern = cache_manager.generate_key(
            CACHE_PREFIX_GRAPHQL, query_hash=query_hash
        )
        return cache_manager.delete_pattern(f"{pattern}*")
    else:
