...
def chrome_devtools_well_known(request):
    """Return empty 204 for Chrome DevTools /.well-known request to avoid 404 WARNING logs."""
    return HttpResponse(status=204)


def worker_source_map_suppress(request):
    """Return 204 for *.worker.js.map requests (e.g. from Swagger UI / Monaco) to suppress 404 logs."""
    return HttpResponse(status=204)


def favicon_empty(request):
    """Return 204 for /favicon.ico to avoid 404 WARNING logs when no favicon is configured."""
    return HttpResponse(status=204)
    ...
    if request.method == 'POST':
        ...
        if validation_errors:
            ...
            try:
                ...
                # Set auth cookies; redirect to login with next= so browser sends cookie on next request
                target = '/'
                response = redirect(settings.LOGIN_URL + '?next=' + quote(target))
                _set_auth_cookies(response, access_token, refresh_token, remember_me)
