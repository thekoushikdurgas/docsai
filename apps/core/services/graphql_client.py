...
class GraphQLError(Exception):
    ...
    Per 01_AUTH_MODULE, errors may include extensions: code, statusCode, fieldErrors.
    """
...
class GraphQLClient:
    ...
    def __init__(self, endpoint_url: Optional[str] = None, access_token: Optional[str] = None, request=None):
        ...
        logger.debug(f"GraphQLClient initialized with endpoint: {self.endpoint}")
        ...
        for attempt in range(self.max_retries):
            try:
                ...
                if 'errors' in result:
                    ...
                    field_errors = ext.get('fieldErrors')
                    if field_errors is not None and not isinstance(field_errors, dict):
                        field_errors = {}
                    raise GraphQLError(
            ...
            try:
                ...
                if 'errors' in result:
                    ...
                    if field_errors is not None and not isinstance(field_errors, dict):
                        field_errors = {}
                    raise GraphQLError(
                        f"GraphQL mutation failed: {', '.join(error_messages)}",
                        code=code,
                        status_code=status_code,
                        field_errors=field_errors,
                        extensions=ext,
