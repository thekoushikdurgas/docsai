...
class APIClient:
    ...
    def _validate_request_data(
        self, data: Optional[Dict[str, Any]], method: str
        ...
        if method.upper() in ("POST", "PUT", "PATCH"):
            ...
            if not isinstance(data, dict):
                raise ValueError(
                    f"Request data must be a dictionary for {method.upper()} requests"
                )
    ...
    def _retry_request(self, method: str, endpoint: str, **kwargs) -> Any:
        ...
        for attempt in range(self.max_retries):
            try:
                logger.debug(
                    f"{method.upper()} {url} (attempt {attempt + 1}/{self.max_retries})"
                )
                ...
                if 400 <= status_code < 500 and status_code != 429:
                    logger.error(
                        f"Client error {status_code} for {method.upper()} {url}: {e}"
                    )
