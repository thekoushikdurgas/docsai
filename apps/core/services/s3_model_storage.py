...
class S3ModelStorage:
    ...
    def _update_index_item(
        self, item_uuid: str, metadata: Dict[str, Any], operation: str = "update"
    ) -> None:
        """
    ...
    def create(
        self, data: Dict[str, Any], item_uuid: Optional[str] = None
    ) -> Dict[str, Any]:
        """
        ...
        logger.debug(f"Created {self.model_name} item: {item_uuid}")
        return data
    ...
    def update(self, item_uuid: str, data: Dict[str, Any]) -> Optional[Dict[str, Any]]:
        ...
        logger.debug(f"Updated {self.model_name} item: {item_uuid}")
        return updated_data
    ...
    def delete(self, item_uuid: str) -> bool:
        ...
        logger.debug(f"Deleted {self.model_name} item: {item_uuid}")
        return True
    ...
    def list(
        ...
        # Sort
        if order_by:
            items_metadata.sort(key=lambda x: x.get(order_by, ""), reverse=reverse)
        else:
            # Default: sort by created_at descending (newest first)
            items_metadata.sort(
                key=lambda x: x.get("created_at", ""), reverse=not reverse
            )
    ...
    def batch_create(self, items: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
        ...
        logger.debug(f"Batch created {len(created_items)} {self.model_name} items")
    ...
    def batch_delete(self, item_uuids: List[str]) -> int:
        ...
        logger.debug(f"Batch deleted {deleted_count} {self.model_name} items")
