...
class BaseService:
    ...
    def _set_cache(
        ...
        try:
            ...
            self.logger.debug(
                f"Cached value for key: {cache_key} (TTL: {timeout}s, type: {data_type or 'default'})"
            )
            return True
        except Exception as e:
    ...
    def _clear_cache(self, cache_key: Optional[str] = None) -> bool:
        ...
        try:
            if cache_key:
                ...
                self.logger.warning(
                    "Clearing all cache - this may affect other services"
                )
            return True
        except Exception as e:
    ...
    def _handle_error(
        self, error: Exception, context: str = "", record_monitoring: bool = True
    ) -> Dict[str, Any]:
        """
    ...
    def _format_list_response(
        ...
        response = {"items": items, "total": total, "offset": offset}
