...
class GraphQLDocumentationService:
    ...
    def __init__(self, request=None, access_token: Optional[str] = None):
        ...
        self.client = GraphQLClient(
            endpoint_url=endpoint_url, access_token=access_token, request=request
        )
    ...
    def _log_operation(
        ...
        **kwargs: Any,
        ...
        if success:
            logger.debug("GraphQL operation completed: %s", log_data, extra=log_data)
        else:
    ...
    def get_page(
        ...
        try:
            result = self.client.execute_query(
                query, variables, use_cache=True, cache_timeout=300
            )
    ...
    def list_pages(
        ...
        try:
            ...
            self.error_count += 1
            self._log_operation(
                "list_pages", "query", duration, False, str(e), page_type=page_type
            )
            return {"pages": [], "total": 0}
        except Exception as e:
            duration = time.time() - start_time
    ...
    def create_page(self, page_data: Dict[str, Any]) -> Optional[Dict[str, Any]]:
        ...
        input_data = {
            ...
            "category": page_data.get("metadata", {}).get("category")
            or page_data.get("category"),
    ...
    def update_page(
        self, page_id: str, page_data: Dict[str, Any]
    ) -> Optional[Dict[str, Any]]:
        """
        ...
        if "description" in metadata or "description" in page_data:
            input_data["description"] = metadata.get("description") or page_data.get(
                "description"
            )
        if "category" in metadata or "category" in page_data:
            input_data["category"] = metadata.get("category") or page_data.get(
                "category"
            )
        ...
        try:
            ...
            if result and "documentation" in result:
                ...
                self._log_operation(
                    "update_page", "mutation", duration, True, page_id=page_id
                )
                return updated_page

            self._log_operation(
                "update_page", "mutation", duration, False, "No result", page_id=page_id
            )
            return None
        except GraphQLError as e:
            duration = time.time() - start_time
            self.error_count += 1
            self._log_operation(
                "update_page", "mutation", duration, False, str(e), page_id=page_id
            )
            return None
        except Exception as e:
            duration = time.time() - start_time
    ...
    def delete_page(self, page_id: str) -> bool:
        ...
        try:
            ...
            if result and "documentation" in result:
                ...
                self._log_operation(
                    "delete_page", "mutation", duration, success, page_id=page_id
                )
                return success

            self._log_operation(
                "delete_page", "mutation", duration, False, "No result", page_id=page_id
            )
            return False
        except GraphQLError as e:
            duration = time.time() - start_time
            self.error_count += 1
            self._log_operation(
                "delete_page", "mutation", duration, False, str(e), page_id=page_id
            )
            return False
        except Exception as e:
            duration = time.time() - start_time
