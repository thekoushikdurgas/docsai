...
class IndexGeneratorService:
    ...
    def __init__(self) -> None:
        from apps.documentation.services import get_shared_s3_storage, get_shared_s3_index_manager
    ...
    def generate_pages_index(self, use_parallel: bool = True) -> Dict[str, Any]:
        ...
        try:
            ...
            stats: Dict[str, Any] = {"total": 0, "by_type": {pt: 0 for pt in PAGE_TYPES}, "by_page_state": {ps: 0 for ps in page_states}}
            ...
            for s3_key in keys:
                ...
                page_state = (metadata.get("page_state", "") if isinstance(metadata, dict) else "") or "draft"
                if page_state not in stats["by_page_state"]:
                    stats["by_page_state"][page_state] = 0
                stats["by_page_state"][page_state] += 1
                file_name = s3_key.split("/")[-1]
                pages.append({"page_id": pid, "page_type": ptype, "route": route, "file_name": file_name})
            ...
            return {"success": True, "index_path": f"{self.data_prefix}pages/index.json", "total": len(pages)}
    ...
    def generate_endpoints_index(self, use_parallel: bool = True) -> Dict[str, Any]:
        ...
        try:
            ...
            for s3_key in keys:
                ...
                if not isinstance(data, dict):
                    continue
                eid = data.get("endpoint_id") or s3_key.split("/")[-1].replace(".json", "")
            ...
            logger.debug("Generated endpoints index with %d endpoints from S3", len(endpoints))
            return {"success": True, "index_path": f"{self.data_prefix}endpoints/index.json", "total": len(endpoints)}
        except Exception as e:
        ...
        try:
            ...
            for sub in ("collections", "environments", "configurations"):
                ...
                for s3_key in keys:
                    ...
                    try:
                        if sub == "collections":
                            name = (data.get("info") or {}).get("name") or file_name.replace(".json", "")
                            collections.append({"collection_id": name, "file_name": file_name, "type": "collection"})
                        elif sub == "environments":
                            name = data.get("name") or file_name.replace(".json", "")
                            environments.append({"environment_id": name, "file_name": file_name, "type": "environment"})
                        else:
                            cid = data.get("config_id") or data.get("name") or file_name.replace(".json", "")
            ...
            logger.debug(
                ...
                len(collections), len(environments), len(configurations),
            )
            return {
                "success": True,
    ...
    def generate_relationships_index(self) -> Dict[str, Any]:
        ...
        try:
            ...
            logger.debug("Generated relationships index with %d entries from S3", len(relationships))
            return {
                "success": True,
