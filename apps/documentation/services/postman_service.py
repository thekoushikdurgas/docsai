...
class PostmanService(DocumentationServiceBase):
    ...
    def list_configurations(
        ...
        if use_cache:
            cache_key = self._get_cache_key(
                "list_configurations", limit=limit, offset=offset, state=state
            )
        ...
        try:
            ...
            if configurations_list:
                ...
                logger.debug(
                    f"Loaded {len(paginated)} configurations from S3 index (total: {total})"
                )
                return {"configurations": paginated, "total": total, "source": "s3"}
        ...
        try:
            ...
            file_keys = s3_storage.list_json_files(
                configurations_prefix, max_keys=10000
            )
            ...
            if configurations:
                self.logger.debug(
                    f"Loaded {len(configurations)} configurations from S3"
                )
                result = {
                ...
                if use_cache:
                    ...
                    self._set_cache(
                        cache_key, result, timeout=120, data_type="list"
                    )  # 2 minutes for list operations (Task 2.2.2)
                return result
        except Exception as e:
        ...
        if not _NO_CONFIGS_LOGGED:
            logger.debug(
                "All strategies failed to load Postman configurations; returning empty result"
            )
            _NO_CONFIGS_LOGGED = True
        return {"configurations": [], "total": 0, "source": "none"}

    def get_configuration(
        self, config_id: str, use_cache: bool = True
    ...
    def get_statistics(self) -> Dict[str, Any]:
        ...
        try:
            ...
            error_response = self._handle_error(
                e, context="Failed to get Postman statistics", record_monitoring=True
            )
            raise DocumentationError(
        ...
        try:
            ...
            if default_collection_file:
                try:
                    ...
                    data_prefix = settings.S3_DATA_PREFIX
                    collection_key = (
                        f"{data_prefix}postman/collection/{default_collection_file}"
                    )
    ...
    def get_environment(
        self, config_id: str, env_name: str
    ) -> Optional[Dict[str, Any]]:
        """
        ...
        try:
            ...
            if not isinstance(environments, list):
                if isinstance(environments, str):
                    ...
                    self.logger.warning(
                        f"Invalid environments format for config {config_id}: {type(environments)}"
                    )
                    return None
            ...
            if (
                environments
                and len(environments) > 0
                and isinstance(environments[0], str)
            ):
                for env_file_name in environments:
                    try:
                        ...
                        if env_data and (
                            env_data.get("name") == env_name
                            or env_data.get("env_name") == env_name
                        ):
                            return env_data
                    except Exception as e:
                ...
                for env in environments:
                    if isinstance(env, dict) and (
                        env.get("name") == env_name or env.get("env_name") == env_name
                    ):
                        return env
                return None
        except Exception as e:
    ...
    def create_configuration(
        ...
        try:
            ...
            if result:
                ...
                self.logger.debug(
                    f"Cleared cache for configuration {config_id} and all postman lists after create"
                )
    ...
    def update_configuration(
        ...
        try:
            ...
            if result:
                ...
                self.unified_storage.clear_cache("postman")
                self.logger.debug(
                    f"Cleared cache for configuration {config_id} and all postman lists after update"
                )
    ...
    def delete_configuration(self, config_id: str) -> bool:
        ...
        try:
            ...
            if success:
                ...
                self.unified_storage.clear_cache("postman")
                self.logger.debug(
                    f"Cleared cache for configuration {config_id} and all postman lists after delete"
                )
    ...
    def _clear_cache_for_configuration(self, config_id: str) -> None:
        ...
        try:
            ...
            self.logger.warning(
                f"Failed to clear cache for configuration {config_id}: {e}"
            )
    ...
    def update_environment(
        self, environment_id: str, environment_data: Dict[str, Any]
    ) -> Optional[Dict[str, Any]]:
        """
        Update environment.
        ...
        try:
            ...
            def _update_environment_with_retry():
                return self.repository.update_environment(
                    environment_id, environment_data
                )
