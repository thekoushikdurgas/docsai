...
class PagesService(DocumentationServiceBase):
    ...
    def get_page(
        self, page_id: str, page_type: Optional[str] = None, use_cache: bool = True
    ...
    def get_page_sections(self, page_id: str) -> Optional[Any]:
        ...
        if not page:
            return None
        return page.get("sections") or (page.get("metadata") or {}).get(
            "content_sections"
        )
    ...
    def get_pages_statistics(self) -> Dict[str, Any]:
        ...
        try:
            ...
            for page in all_pages:
                ...
                state = metadata.get("page_state") or metadata.get(
                    "status", "published"
                )
            ...
            error_response = self._handle_error(
                e, context="Failed to get pages statistics", record_monitoring=True
            )
    ...
    def create_page(self, page_data: Dict[str, Any]) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            self.logger.error(
                f"Data transformation error in create_page: {e}", exc_info=True
            )
        ...
        try:
            result = self._create_resource(
                resource_data=lambda_data, operation_name="create_page"
            )
    ...
    def update_page(
        ...
        try:
            ...
            updated_data = DataTransformer.handle_partial_update(
                existing, lambda_updates, "page"
            )
        except Exception as e:
            self.logger.error(
                f"Data transformation error in update_page: {e}", exc_info=True
            )
            raise ValueError(f"Invalid page data: {e}") from e
        ...
        try:
            ...
            error_response = self._handle_error(
                e, context=f"Failed to update page {page_id}", record_monitoring=True
            )
            raise DocumentationError(
        ...
        try:
            ...
            if self.graphql_service:
                try:
                    ...
                    self.logger.warning(
                        f"GraphQL failed for delete_page, falling back: {e}"
                    )

            # Fallback to base class delete method (S3 direct)
            result = self._delete_resource(
                resource_id=page_id, operation_name="delete_page"
            )
            return result
            ...
            error_response = self._handle_error(
                e, context=f"Failed to delete page {page_id}", record_monitoring=True
            )
            raise DocumentationError(
