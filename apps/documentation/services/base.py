...
class DocumentationServiceBase(BaseService, Generic[T]):
    ...
    def __init__(
        ...
        self.resource_name = resource_name or service_name.lower().replace(
            "service", ""
        )
        ...
        **kwargs,
        ...
        try:
            ...
            if use_cache and result is not None:
                cache_key = self._get_cache_key(operation_name, resource_id, **kwargs)
                self._set_cache(
                    cache_key, result, timeout=self.cache_timeout, data_type="detail"
                )
    ...
    def _list_resources(
        self, operation_name: str, storage_method: str, use_cache: bool = True, **kwargs
    ) -> Dict[str, Any]:
        """
    ...
    def _create_resource(
        ...
        try:
            if repository_method:
                ...
                method_name = (
                    f"create_{self.resource_name.rstrip('s')}"  # e.g., "create_page"
                )
            ...
            if result:
                resource_id = result.get("id") or result.get(
                    f"{self.resource_name.rstrip('s')}_id"
                )
                if resource_id:
            ...
            self.logger.debug(
                f"{operation_name} created successfully: {resource_data.get('id')}"
            )
            return result
    ...
    def _update_resource(
        ...
        try:
            if repository_method:
                ...
                method_name = (
                    f"update_{self.resource_name.rstrip('s')}"  # e.g., "update_page"
                )
                method = getattr(self.repository, method_name, None)
                if not method:
    ...
    def _delete_resource(
        ...
        try:
            if repository_method:
                ...
                method_name = (
                    f"delete_{self.resource_name.rstrip('s')}"  # e.g., "delete_page"
                )
                method = getattr(self.repository, method_name, None)
                if not method:
        ...
        try:
            ...
            if hasattr(self, "unified_storage") and self.unified_storage:
                ...
                cache_key = self._get_cache_key(
                    f"get_{self.resource_name.rstrip('s')}", resource_id
                )
