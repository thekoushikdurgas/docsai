...
class ApiTrackingMiddleware:
    ...
    def __init__(self, get_response: Callable[[HttpRequest], HttpResponse]) -> None:
        ...
        self.user_type_enabled = getattr(
            settings, "API_TRACKING_USER_TYPE_ENABLED", True
        )
    ...
    def __call__(self, request: HttpRequest) -> HttpResponse:
        if (
            ...
            or not request.path.startswith(self.prefix)
        ):
        ...
        if endpoint_key:
            try:
                ...
                if self.user_type_enabled:
                    ...
                    if user_type:
                        record_hit_with_user_type(
                            endpoint_key, user_type, response.status_code, duration_ms
                        )
                        logger.debug(
                            "Tracked %s for user_type=%s", endpoint_key, user_type
                        )
