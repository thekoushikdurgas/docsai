...
class ValidationRules:
    ...
    def validate_page_type(value: str) -> str:
        ...
        if value not in ValidationRules.PAGE_TYPES:
            raise ValidationError(
                _(f"page_type must be one of {ValidationRules.PAGE_TYPES}")
            )
        return value
    ...
    def validate_page_state(value: str) -> str:
        ...
        if value not in ValidationRules.PAGE_STATES:
            raise ValidationError(
                _(f"page_state must be one of {ValidationRules.PAGE_STATES}")
            )
        return value
    ...
    def validate_status(value: str) -> str:
        ...
        if value not in ValidationRules.PAGE_STATUSES:
            raise ValidationError(
                _(f"status must be one of {ValidationRules.PAGE_STATUSES}")
            )
        return value
    ...
    def validate_method(value: str) -> str:
        ...
        if value.upper() not in ValidationRules.HTTP_METHODS:
            raise ValidationError(
                _(f"method must be one of {ValidationRules.HTTP_METHODS}")
            )
        return value.upper()
    ...
    def validate_usage_type(value: str) -> str:
        ...
        if value not in ValidationRules.USAGE_TYPES:
            raise ValidationError(
                _(f"usage_type must be one of {ValidationRules.USAGE_TYPES}")
            )
        return value
    ...
    def validate_usage_context(value: str) -> str:
        ...
        if value not in ValidationRules.USAGE_CONTEXTS:
            raise ValidationError(
                _(f"usage_context must be one of {ValidationRules.USAGE_CONTEXTS}")
            )
        return value
    ...
    def validate_invocation_pattern(value: str) -> str:
        ...
        if value not in ValidationRules.INVOCATION_PATTERNS:
            raise ValidationError(
                _(
                    f"invocation_pattern must be one of {ValidationRules.INVOCATION_PATTERNS}"
                )
            )
        return value
    ...
    def validate_error_handling(value: str) -> str:
        ...
        if value not in ValidationRules.ERROR_HANDLING_STRATEGIES:
            raise ValidationError(
                _(
                    f"error_handling must be one of {ValidationRules.ERROR_HANDLING_STRATEGIES}"
                )
            )
        return value
    ...
    def validate_loading_state(value: str) -> str:
        ...
        if value not in ValidationRules.LOADING_STATES:
            raise ValidationError(
                _(f"loading_state must be one of {ValidationRules.LOADING_STATES}")
            )
        return value
...
def validate_required_fields(data: Dict[str, Any], required_fields: List[str]) -> None:
    ...
    if missing:
        raise ValidationError(
            {
                field: ValidationErrorCodes.get_message(
...
def validate_computed_fields(
    data: Dict[str, Any], computed_fields: Dict[str, Any]
) -> Dict[str, Any]:
...
def validate_page_metadata(data: Dict[str, Any]) -> Dict[str, Any]:
    ...
    validate_required_fields(
        data, ["route", "file_path", "purpose", "s3_key", "last_updated"]
    )
        ...
        for endpoint in data["uses_endpoints"]:
            if isinstance(endpoint, dict):
                ...
                if "usage_type" in endpoint:
                    endpoint["usage_type"] = rules.validate_usage_type(
                        endpoint["usage_type"]
                    )
                if "usage_context" in endpoint:
                    endpoint["usage_context"] = rules.validate_usage_context(
                        endpoint["usage_context"]
                    )
...
def validate_endpoint_documentation(data: Dict[str, Any]) -> Dict[str, Any]:
    ...
    # Validate required fields
    validate_required_fields(
        data, ["endpoint_id", "endpoint_path", "method", "api_version", "description"]
    )
    ...
    if "endpoint_state" not in data:
        ...
        data["endpoint_state"] = rules.validate_page_state(
            data["endpoint_state"]
        )  # Same values
    ...
    if not (has_legacy_file or has_files_object):
        raise ValidationError(
            {
                "service_file": ValidationErrorCodes.get_message(
                    ValidationErrorCodes.MISSING_FILE_REFERENCE
                ),
                "router_file": ValidationErrorCodes.get_message(
                    ValidationErrorCodes.MISSING_FILE_REFERENCE
                ),
            }
        )
        ...
        for page_usage in data["used_by_pages"]:
            if isinstance(page_usage, dict):
                if "usage_type" in page_usage:
                    page_usage["usage_type"] = rules.validate_usage_type(
                        page_usage["usage_type"]
                    )
                if "usage_context" in page_usage:
                    page_usage["usage_context"] = rules.validate_usage_context(
                        page_usage["usage_context"]
                    )
...
def validate_relationship_connection(data: Dict[str, Any]) -> Dict[str, Any]:
    ...
    if "invocation_pattern" not in data:
        ...
        data["invocation_pattern"] = rules.validate_invocation_pattern(
            data["invocation_pattern"]
        )
