        ...
        def wrapper(request, *args, **kwargs):
            try:
                ...
                if result is None:
                    error_msg = (
                        not_found_error
                        or f"{operation_name.split()[0].title()} not found"
                    )
                    return JsonResponse(
                        {"success": False, "error": error_msg}, status=404
                    )
                ...
                return JsonResponse(
                    {"success": True, "data": result}, status=success_status
                )
...
def update_api_endpoint(resource_name, update_func):
    ...
    @api_endpoint(
        f"update {resource_name}", not_found_error=f"{resource_name.title()} not found"
    )
...
def delete_api_endpoint(resource_name, delete_func):
    ...
    @api_endpoint(
        f"delete {resource_name}", not_found_error=f"{resource_name.title()} not found"
    )
