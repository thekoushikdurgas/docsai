    ...
    def validate(
        ...
        for field in self.REQUIRED_FIELDS:
            if field not in data:
                errors.append(
                    SchemaValidationError(field, f"Required field '{field}' is missing")
                )
        if errors:
            return False, errors
            ...
            if not isinstance(page_id, str):
                errors.append(
                    SchemaValidationError(
                        "page_id", "page_id must be a string", page_id
                    )
                )
                ...
                errors.append(
                    SchemaValidationError(
                        "page_id",
                        "page_id must be alphanumeric with underscores/hyphens only",
                        page_id,
                    )
                )
        ...
        if page_type and page_type not in self.VALID_PAGE_TYPES:
            errors.append(
                SchemaValidationError(
                    "page_type",
                    f"page_type must be one of {self.VALID_PAGE_TYPES}",
                    page_type,
                )
            )
            ...
            if not isinstance(metadata, dict):
                ...
                if route:
                    if not isinstance(route, str):
                        errors.append(
                            SchemaValidationError(
                                "metadata.route", "route must be a string", route
                            )
                        )
                    elif not route.startswith("/") and route != "/":
                        errors.append(
                            SchemaValidationError(
                                "metadata.route", "route must start with '/'", route
                            )
                        )
                ...
                if uses_endpoints:
                    if not isinstance(uses_endpoints, list):
                        errors.append(
                            SchemaValidationError(
                                "metadata.uses_endpoints",
                                "uses_endpoints must be an array",
                                uses_endpoints,
                            )
                        )
                    else:
                            ...
                            if not isinstance(endpoint, dict):
                                ...
                                for f in required_endpoint_fields:
                                    if f not in endpoint:
                                        errors.append(
                                            SchemaValidationError(
                                                f"metadata.uses_endpoints[{idx}].{f}",
                    ...
                    if not isinstance(ui_components, list):
                        errors.append(
                            SchemaValidationError(
                                ...
                                "ui_components must be an array",
                                ui_components,
                            )
                        )
                    else:
                        for idx, component in enumerate(ui_components):
                            if not isinstance(component, dict):
                                ...
                                if "name" not in component:
                                    errors.append(
                                        SchemaValidationError(
                                            f"metadata.ui_components[{idx}].name",
                                ...
                                if "file_path" not in component:
                                    errors.append(
                                        SchemaValidationError(
                                            f"metadata.ui_components[{idx}].file_path",
                                            "Component file_path is required",
                                        )
                                    )
                ...
                if isinstance(uses_endpoints, list) and endpoint_count != len(
                    uses_endpoints
                ):
                    errors.append(
                        SchemaValidationError(
                    ...
                    if not isinstance(api_versions, list):
                        ...
                        for idx, version in enumerate(api_versions):
                            if not isinstance(version, str):
                                errors.append(
                                    SchemaValidationError(
                                        f"metadata.api_versions[{idx}]",
                                        "api_versions item must be a string",
                                        version,
                                    )
                                )
...
class EndpointSchemaValidator:
    ...
    def validate(
        ...
        if endpoint_id and not isinstance(endpoint_id, str):
            errors.append(
                SchemaValidationError(
                    "endpoint_id", "endpoint_id must be a string", endpoint_id
                )
            )
        ...
        if endpoint_path and not isinstance(endpoint_path, str):
            errors.append(
                SchemaValidationError(
                    "endpoint_path", "endpoint_path must be a string", endpoint_path
                )
            )
        ...
        if method and method not in self.VALID_METHODS:
            errors.append(
                SchemaValidationError(
                    "method", f"method must be one of {self.VALID_METHODS}", method
                )
            )
        ...
        if api_version and not isinstance(api_version, str):
            errors.append(
                SchemaValidationError(
                    "api_version", "api_version must be a string", api_version
                )
            )
        ...
        if graphql_operation and not isinstance(graphql_operation, str):
            errors.append(
                SchemaValidationError(
                    "graphql_operation",
                    "graphql_operation must be a string",
                    graphql_operation,
                )
            )
        for list_field, item_desc in (
            ...
            if items:
                if not isinstance(items, list):
                    errors.append(
                        SchemaValidationError(
                            list_field, f"{list_field} must be an array", items
                        )
                    )
                else:
        ...
        if used_by_pages:
            if not isinstance(used_by_pages, list):
                errors.append(
                    SchemaValidationError(
                        "used_by_pages", "used_by_pages must be an array", used_by_pages
                    )
                )
            else:
                for idx, page in enumerate(used_by_pages):
                    if not isinstance(page, dict):
                        errors.append(
                            SchemaValidationError(
                                f"used_by_pages[{idx}]",
                                "Page reference must be an object",
                                page,
                            )
                        )
        ...
        if isinstance(used_by_pages, list) and page_count != len(used_by_pages):
            errors.append(
                SchemaValidationError(
                    "page_count",
                    f"page_count ({page_count}) should match used_by_pages length ({len(used_by_pages)})",
                    page_count,
                )
            )
        ...
        for field in ["created_at", "updated_at"]:
            if field in data:
                ...
                if not isinstance(value, str):
                    errors.append(
                        SchemaValidationError(
                            field, f"{field} must be an ISO 8601 string", value
                        )
                    )
                else:
                    try:
...
class RelationshipSchemaValidator:
    ...
    def _check_timestamps(
        self, data: Dict[str, Any], errors: List[SchemaValidationError]
    ) -> None:
        for field in ["created_at", "updated_at"]:
            if field in data:
                value = data[field]
    ...
    def validate_by_page(
        ...
        if "page_path" not in data:
            errors.append(
                SchemaValidationError(
                    "page_path", "Required field 'page_path' is missing"
                )
            )
        if "endpoints" not in data:
            errors.append(
                SchemaValidationError(
                    "endpoints", "Required field 'endpoints' is missing"
                )
            )
        if errors:
            return False, errors
        ...
        if page_path:
            if not isinstance(page_path, str):
                errors.append(
                    SchemaValidationError(
                        "page_path", "page_path must be a string", page_path
                    )
                )
            elif not page_path.startswith("/") and page_path != "/":
                errors.append(
                    SchemaValidationError(
                        "page_path", "page_path must start with '/'", page_path
                    )
                )
        ...
        if endpoints:
            if not isinstance(endpoints, list):
                errors.append(
                    SchemaValidationError(
                        "endpoints", "endpoints must be an array", endpoints
                    )
                )
            else:
                required = [
                        ...
                        if ut and ut not in self.VALID_USAGE_TYPES:
                            errors.append(
                                SchemaValidationError(
                                    ...
                                    f"usage_type must be one of {self.VALID_USAGE_TYPES}",
                                    ut,
                                )
                            )
                        uc = endpoint.get("usage_context")
                        if uc and uc not in self.VALID_USAGE_CONTEXTS:
                            errors.append(
                                SchemaValidationError(
                                    ...
                                    f"usage_context must be one of {self.VALID_USAGE_CONTEXTS}",
                                    uc,
                                )
                            )
    ...
    def validate_by_endpoint(
        ...
        if "endpoint_path" not in data:
            errors.append(
                SchemaValidationError(
                    "endpoint_path", "Required field 'endpoint_path' is missing"
                )
            )
        if "method" not in data:
            errors.append(
                SchemaValidationError("method", "Required field 'method' is missing")
            )
        if "pages" not in data:
            errors.append(
                SchemaValidationError("pages", "Required field 'pages' is missing")
            )
        if errors:
            return False, errors
        ...
        if pages:
            if not isinstance(pages, list):
                errors.append(
                    SchemaValidationError("pages", "pages must be an array", pages)
                )
            else:
                required = [
                    "page_path",
                        ...
                        if ut and ut not in self.VALID_USAGE_TYPES:
                            errors.append(
                                SchemaValidationError(
                                    ...
                                    f"usage_type must be one of {self.VALID_USAGE_TYPES}, got {ut}",
                                    ut,
                                )
                            )
                        ...
                        if uc and uc not in self.VALID_USAGE_CONTEXTS:
                            errors.append(
                                SchemaValidationError(
                                    ...
                                    f"usage_context must be one of {self.VALID_USAGE_CONTEXTS}, got {uc}",
                                    uc,
                                )
                            )
