...
class S3IndexManager:
    ...
    def read_index(
        self, resource_type: str, use_cache: bool = True, cache_ttl: int = 300
    ) -> Dict[str, Any]:
        """
    ...
    def update_index(self, resource_type: str, data: Dict[str, Any]) -> bool:
        ...
        try:
            ...
            try:
                ...
                logger.debug(
                    f"Invalidated UnifiedStorage list cache for {resource_type}"
                )
            except Exception as e:
                logger.warning(
                    f"Failed to invalidate UnifiedStorage cache for {resource_type}: {e}"
                )

            logger.debug(
                f"Updated index for {resource_type} and invalidated all related caches"
            )
            return True
        except Exception as e:
    ...
    def add_item_to_index(
        self, resource_type: str, item_id: str, item_data: Dict[str, Any]
    ) -> bool:
        """
        ...
        try:
            ...
            if resource_type == "pages":
                ...
                route = item_data.get("metadata", {}).get("route") or item_data.get(
                    "route", ""
                )
                summary = {
                    "page_id": item_id,
            ...
            logger.error(
                f"Failed to add item to index for {resource_type}: {e}", exc_info=True
            )
            return False

    def remove_item_from_index(self, resource_type: str, item_id: str) -> bool:
        """
        ...
        try:
            ...
            logger.error(
                f"Failed to remove item from index for {resource_type}: {e}",
                exc_info=True,
            )
            return False
    ...
    def validate_index(self, resource_type: str) -> Dict[str, Any]:
        ...
        try:
            ...
            logger.error(
                f"Failed to validate index for {resource_type}: {e}", exc_info=True
            )
            return {
                "valid": False,
                "total_in_s3": 0,
    ...
    def get_index_health(
        self, resource_type: str, max_age_hours: int = 24
    ) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            if is_stale:
                warnings.append(
                    f"Index is stale (age: {age_hours:.1f} hours, max: {max_age_hours} hours)"
                )
            ...
            logger.error(
                f"Failed to check index health for {resource_type}: {e}", exc_info=True
            )
            return {
                "healthy": False,
                "age_hours": None,
    ...
    def rebuild_index(self, resource_type: str) -> bool:
        ...
        try:
            ...
            for file_key in json_files:
                ...
                if resource_type == "pages":
                    page_type = item_data.get("page_type", "docs")
                    status = item_data.get("metadata", {}).get(
                        "status"
                    ) or item_data.get("status", "published")
                    route = item_data.get("metadata", {}).get("route") or item_data.get(
                        "route", "/"
                    )
