...
class DataTransformer:
    ...
    @staticmethod
    def _normalize_data_reference_list(
        raw_list: List[Any], list_name: str
    ) -> List[Dict[str, Any]]:
        """Normalize a list of fallback/mock/demo items to DataReference-compatible dicts."""
        if not raw_list:
            return []
        return [
            _normalize_data_ref_item(it, list_name, i) for i, it in enumerate(raw_list)
        ]
    ...
    def django_to_lambda_page(django_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if "authentication" not in metadata:
            metadata["authentication"] = django_data.get(
        ...
        lambda_data = {
            ...
            "created_at": django_data.get("created_at")
    ...
    def lambda_to_django_page(lambda_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if isinstance(metadata, dict):
            ...
            django_data["title"] = metadata.get(
                "purpose", ""
            )  # Map purpose to title for form
    ...
    def django_to_lambda_endpoint(django_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        lambda_data = {
            ...
            "created_at": django_data.get("created_at")
            or datetime.now(timezone.utc).isoformat(),
            "updated_at": django_data.get("updated_at")
            or datetime.now(timezone.utc).isoformat(),
    ...
    def lambda_to_django_relationship(lambda_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if "connection" in django_data and django_data["connection"]:
            ...
            if "usage_context" not in django_data:
                django_data["usage_context"] = connection.get(
    ...
    def calculate_computed_fields(
        data: Dict[str, Any], resource_type: str
    ) -> Dict[str, Any]:
    ...
    def handle_partial_update(
        existing: Dict[str, Any], updates: Dict[str, Any], resource_type: str
    ) -> Dict[str, Any]:
