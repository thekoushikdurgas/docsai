"""
Integration tests for the documentation dashboard views.

Tests cover:
- main documentation dashboard view (/docs/)
...
class DocumentationDashboardViewsTestCase(BaseAPITestCase):
    """Test cases for documentation dashboard views."""
    ...
    def setUp(self):
        ...
        self.postman_url = reverse(
            "documentation:dashboard"
        )  # postman is tab on dashboard
    ...
    def test_dashboard_get(self):
        """Test documentation dashboard page GET request."""
        response = self.client.get(self.dashboard_url)
        self.assertEqual(response.status_code, 200)
        self.assertContains(response, "Documentation Dashboard", status_code=200)
    ...
    def test_media_manager_dashboard_with_view_mode(self):
        ...
        for view_mode in ("list", "grid"):
            response = self.client.get(
                self.dashboard_url, {"tab": "pages", "view": view_mode}
            )
    ...
    @patch(
        "apps.documentation.views.documentation_dashboard_views.get_media_manager_dashboard_service"
    )
    ...
    def test_media_manager_page_detail_get(self):
        ...
        with patch(
            "apps.documentation.views.media_manager_dashboard.get_pages_service"
        ) as mock_get_service:
            mock_service = Mock()
            mock_get_service.return_value = mock_service
    ...
    def test_media_manager_endpoint_detail_get(self):
        ...
        detail_url = reverse(
            "documentation:endpoint_detail_enhanced", args=[endpoint_id]
        )

        with patch(
            "apps.documentation.views.media_manager_dashboard.get_endpoints_service"
        ) as mock_get_service:
            mock_service = Mock()
            mock_get_service.return_value = mock_service
    ...
    def test_media_manager_health_get(self):
        ...
        with patch(
            "apps.documentation.views.dashboard.get_comprehensive_health_status"
        ) as mock_health:
