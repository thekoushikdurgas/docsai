...
class DocumentationDashboardAPITestCase(BaseAPITestCase):
    ...
    def setUp(self):
        ...
        self.relationships_api_url = reverse(
            "documentation:api_docs_dashboard_relationships"
        )
    ...
    def test_get_pages_list_api_success(self):
        ...
        with patch(
            "apps.documentation.api.pages_dashboard_api.get_pages_service"
        ) as mock_get_service:
            mock_service = Mock()
            ...
            items = (
                ...
                else data["data"].get("pages", data["data"].get("items", []))
            )
    ...
    def test_get_pages_list_api_with_filters(self):
        ...
        with patch(
            ...
            response = self.client.get(
                self.pages_api_url,
                {"page_type": "docs", "status": "published", "limit": 10, "offset": 0},
            )
    ...
    def test_get_pages_list_api_with_page_types(self):
        ...
        with patch(
            ...
            "apps.core.decorators.auth.cache.get", return_value=True
        ):
            mock_service = Mock()
            mock_get_service.return_value = mock_service
            mock_service.list_pages.return_value = {"pages": mock_pages, "total": 2}
    ...
    def test_get_endpoints_list_api_success(self):
        ...
        with patch(
            "apps.documentation.api.endpoints_dashboard_api.get_endpoints_service"
        ) as mock_get_service:
            mock_service = Mock()
            mock_get_service.return_value = mock_service
            ...
            items = (
                data["data"]
                if isinstance(data["data"], list)
                else data["data"].get("endpoints", data["data"].get("items", []))
            )
            self.assertEqual(len(items), 2)
    ...
    def test_get_relationships_list_api_success(self):
        ...
        with patch(
            "apps.documentation.api.relationships_dashboard_api.get_relationships_service"
        ) as mock_get_service:
            mock_service = Mock()
            mock_get_service.return_value = mock_service
            ...
            items = (
                data["data"]
                if isinstance(data["data"], list)
                else data["data"].get("relationships", data["data"].get("items", []))
            )
            self.assertEqual(len(items), 2)
    ...
    def test_get_postman_list_api_success(self):
        ...
        with patch(
            "apps.documentation.api.postman_dashboard_api.get_postman_service"
        ) as mock_get_service:
            mock_service = Mock()
            mock_get_service.return_value = mock_service
            ...
            items = (
                data["data"]
                if isinstance(data["data"], list)
                else data["data"].get("configurations", data["data"].get("items", []))
            )
            self.assertEqual(len(items), 2)
    ...
    def test_get_statistics_api_success(self):
        ...
        with patch(
            "apps.documentation.api.dashboard_api_common.get_media_manager_dashboard_service"
        ) as mock_get_service:
            mock_service = Mock()
            mock_get_service.return_value = mock_service
            mock_service.pages_service.get_pages_statistics.return_value = {"total": 10}
            mock_service.endpoints_service.get_api_version_statistics.return_value = {
                "versions": []
            }
            mock_service.relationships_service.get_statistics.return_value = {
                "total_relationships": 15
            }
            mock_service.postman_service.get_statistics.return_value = {
    ...
    def test_get_health_api_success(self):
        ...
        with patch(
            "apps.documentation.api.dashboard_api_common.get_comprehensive_health_status",
    ...
    def test_api_endpoints_require_login(self):
        ...
        for url in endpoints:
            ...
            self.assertIn(
                response.status_code, [302, 401], f"Endpoint {url} should require auth"
            )
    ...
    def test_api_endpoints_only_get(self):
        ...
        for url in endpoints:
            ...
            self.assertEqual(
                response.status_code, 405, f"POST to {url} should return 405"
            )

            # Try PUT
            response = self.client.put(url)
            self.assertEqual(
                response.status_code, 405, f"PUT to {url} should return 405"
            )

            # Try DELETE
            response = self.client.delete(url)
            self.assertEqual(
                response.status_code, 405, f"DELETE to {url} should return 405"
            )
