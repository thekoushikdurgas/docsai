...
class PageFactory:
    ...
    def create_batch(count: int, **kwargs) -> List[Dict[str, Any]]:
        ...
        return [
            PageFactory.create(page_id=f"test-page-{i}", **kwargs) for i in range(count)
        ]
...
class RelationshipFactory:
    ...
    def create_batch(count: int, **kwargs) -> List[Dict[str, Any]]:
        ...
        return [
            RelationshipFactory.create(relationship_id=f"test-rel-{i}", **kwargs)
            for i in range(count)
        ]
...
class MockLambdaAPIClient:
    ...
    def set_response(
        ...
        self.responses[key] = {"status_code": status_code, "data": response}
    ...
    def request(
        ...
        **kwargs,
        ...
        self.requests.append(
            {"method": method, "endpoint": endpoint, "data": data, **kwargs}
        )
...
def generate_random_endpoint_data(**overrides) -> Dict[str, Any]:
    ...
    return EndpointFactory.create(endpoint_id=endpoint_id, method=method, **overrides)
...
def generate_random_relationship_data(**overrides) -> Dict[str, Any]:
    ...
    relationship_type = random.choice(
        ["references", "depends_on", "related_to", "includes"]
    )

    return RelationshipFactory.create(relationship_type=relationship_type, **overrides)
