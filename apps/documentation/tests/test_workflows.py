...
class CreatePageWorkflowTestCase(BaseAPITestCase):
    ...
    def test_create_page_workflow_complete(self, mock_pages_service):
        ...
        response = self.client.post(
            self.create_api_url,
            data=json.dumps(
                {"page_id": "new_page", "page_type": "docs", "content": "# New Page"}
            ),
        ...
        mock_pages_service.list_pages.return_value = {
            "pages": [{"page_id": "new_page", "page_type": "docs"}],
            "total": 1,
        }
...
class EditPageWorkflowTestCase(BaseAPITestCase):
    ...
    def setUp(self):
        ...
        self.update_api_url = reverse(
            "documentation:api_page_update", args=[self.page_id]
        )
    ...
    def test_edit_page_workflow_complete(
        self, mock_relationships_service, mock_endpoints_service, mock_pages_service
    ):
        ...
        assert_api_response(
            self, update_response, expected_status=200, expected_success=True
        )
...
class DeletePageWorkflowTestCase(BaseAPITestCase):
    ...
    def setUp(self):
        ...
        super().setUp()
        self.page_id = "test_page"
        self.delete_api_url = reverse(
            "documentation:api_page_delete", args=[self.page_id]
        )
        self.list_url = reverse("documentation:dashboard_pages")
    ...
    def test_delete_page_workflow_complete(self, mock_pages_service):
        ...
        assert_api_response(
            self, delete_response, expected_status=200, expected_success=True
        )
...
class CreateEndpointWorkflowTestCase(BaseAPITestCase):
    ...
    def test_create_endpoint_workflow_complete(self, mock_endpoints_service):
        ...
        mock_endpoints_service.list_endpoints.return_value = {
            "endpoints": [{"endpoint_id": "new_endpoint", "method": "GET"}],
            "total": 1,
        }
...
class CreateRelationshipWorkflowTestCase(BaseAPITestCase):
    ...
    def test_create_relationship_workflow_complete(
        self, mock_endpoints_service, mock_pages_service, mock_relationships_service
    ):
        ...
        response = self.client.post(
            self.create_api_url,
            data=json.dumps(
                {"page_id": "page1", "endpoint_id": "ep1", "usage_type": "primary"}
            ),
            content_type="application/json",
        )
