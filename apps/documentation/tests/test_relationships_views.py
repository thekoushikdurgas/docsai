...
class RelationshipDetailViewTestCase(BaseAPITestCase):
    ...
    def setUp(self):
        ...
        self.detail_url = reverse(
            "documentation:relationship_detail", args=[self.relationship_id]
        )
    ...
    def test_relationship_detail_view_not_found(self, mock_relationships_service):
        ...
        mock_relationships_service.get_relationship.side_effect = NotFoundError(
...
class RelationshipCreateAPITestCase(BaseAPITestCase):
    ...
    @unittest.skip(
        "Create API returns 500 under test; mock vs lazy service needs investigation"
    )
    @patch("apps.documentation.views.relationships_views.relationships_service")
    def test_relationship_create_api_success(self, mock_relationships_service):
        ...
        assert_api_response(self, response, expected_status=201, expected_success=True)
...
class RelationshipUpdateAPITestCase(BaseAPITestCase):
    ...
    def setUp(self):
        ...
        self.update_api_url = reverse(
            "documentation:api_relationship_update", args=[self.relationship_id]
        )
    ...
    def test_relationship_update_api_success(self, mock_relationships_service):
        ...
        response = self.client.patch(
            self.update_api_url,
            data=json.dumps({"usage_type": "secondary"}),
            content_type="application/json",
        )
...
class RelationshipDeleteAPITestCase(BaseAPITestCase):
    ...
    def setUp(self):
        ...
        self.delete_api_url = reverse(
            "documentation:api_relationship_delete", args=[self.relationship_id]
        )
