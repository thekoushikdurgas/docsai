...
class PagesServiceTestCase(TestCase):
    ...
    def setUp(self):
        ...
        self.mock_storage = Mock(
            spec=["get_page", "list_pages", "create_page", "update_page", "delete_page"]
        )
        self.mock_repo = Mock()
        self.service = PagesService(
            unified_storage=self.mock_storage, repository=self.mock_repo
        )
    ...
    def test_list_pages_success(self):
        ...
        self.mock_storage.list_pages.return_value = {"pages": pages, "total": 5}
    ...
    def test_list_pages_with_filters(self):
        ...
        # Create mixed pages
        docs_pages = PageFactory.create_batch(
            3, page_type="docs", page_state="published"
        )
        dashboard_pages = PageFactory.create_batch(
            2, page_type="dashboard", page_state="draft"
        )

        # Mock storage to return filtered results
        self.mock_storage.list_pages.return_value = {"pages": docs_pages, "total": 3}
...
class EndpointsServiceTestCase(TestCase):
    ...
    def test_create_endpoint(self, mock_repo_class):
        ...
        mock_repo = Mock()
        mock_repo.create.return_value = {"endpoint_id": "new-ep", "created": True}
        mock_repo_class.return_value = mock_repo
...
class RelationshipsServiceTestCase(TestCase):
    ...
    def test_create_relationship(self, mock_repo_class):
        ...
        mock_repo = Mock()
        mock_repo.create.return_value = {"relationship_id": "new-rel", "created": True}
        mock_repo_class.return_value = mock_repo
...
class PostmanServiceTestCase(TestCase):
    ...
    def test_get_postman_collection(self, mock_storage_class):
        ...
        mock_lambda_client.get_postman_collection_v1.return_value = {
            "collection": {"info": {"name": "Test Collection"}, "item": []}
        }
        mock_storage.lambda_client = mock_lambda_client
        mock_storage_class.return_value = mock_storage
        ...
        mock_lambda_client.get_postman_collection_v1.assert_called_once_with(
            "test-collection"
        )
    ...
    def test_create_postman_collection(self, mock_storage_class):
        ...
        self.assertIsNotNone(result)
        mock_lambda_client.get_postman_configuration_v1.assert_called_once_with(
            "new-collection"
        )
