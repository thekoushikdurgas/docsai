...
class LambdaModelValidator:
    ...
    def validate_method(value: str) -> str:
        ...
        if value.upper() not in valid_methods:
            raise ValidationError(
                _(f"method must be one of {valid_methods}, got {value}")
            )
        return value.upper()
    ...
    def validate_page_type(value: str) -> str:
        ...
        if value not in PAGE_TYPES:
            raise ValidationError(
                _(f"page_type must be one of {list(PAGE_TYPES)}, got {value}")
            )
        return value
    ...
    def validate_page_state(value: str) -> str:
        ...
        if value not in valid_states:
            raise ValidationError(
                _(f"page_state must be one of {valid_states}, got {value}")
            )
        return value
    ...
    def validate_status(value: str) -> str:
        ...
        if value not in valid_statuses:
            raise ValidationError(
                _(f"status must be one of {valid_statuses}, got {value}")
            )
        return value
    ...
    def validate_endpoint_state(value: str) -> str:
        ...
        if value not in valid_states:
            raise ValidationError(
                _(f"endpoint_state must be one of {valid_states}, got {value}")
            )
        return value
    ...
    def validate_usage_type(value: str) -> str:
        ...
        if value not in valid_types:
            raise ValidationError(
                _(f"usage_type must be one of {valid_types}, got {value}")
            )
        return value
    ...
    def validate_usage_context(value: str) -> str:
        ...
        if value not in valid_contexts:
            raise ValidationError(
                _(f"usage_context must be one of {valid_contexts}, got {value}")
            )
        return value
    ...
    def validate_relationship_state(value: str) -> str:
        ...
        if value not in valid_states:
            raise ValidationError(
                _(f"state must be one of {valid_states}, got {value}")
            )
        return value
...
class EndpointDocumentationValidator:
    ...
    def validate(data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if "endpoint_state" in data:
            data["endpoint_state"] = validator.validate_endpoint_state(
                data["endpoint_state"]
            )
        else:
        ...
        has_files_object = files and (
            files.get("service_file") or files.get("router_file")
        )
        if not (has_legacy_file or has_files_object):
            raise ValidationError(
                _("At least one of 'service_file' or 'router_file' must be provided")
            )
...
class RelationshipValidator:
    ...
    def validate(data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if "usage_context" in data:
            data["usage_context"] = validator.validate_usage_context(
                data["usage_context"]
            )
...
def validate_page_data(page_data: Dict[str, Any]) -> Dict[str, Any]:
    ...
    try:
        ...
        raise ValidationError(
            _("Invalid page data: %(error)s") % {"error": str(e)}
        ) from e
...
def validate_endpoint_data(endpoint_data: Dict[str, Any]) -> Dict[str, Any]:
    ...
    try:
        ...
        raise ValidationError(
            _("Invalid endpoint data: %(error)s") % {"error": str(e)}
        ) from e
...
def validate_relationship_data(relationship_data: Dict[str, Any]) -> Dict[str, Any]:
    ...
    try:
        ...
        raise ValidationError(
            _("Invalid relationship data: %(error)s") % {"error": str(e)}
        ) from e
...
def validate_postman_configuration_data(config_data: Dict[str, Any]) -> Dict[str, Any]:
    ...
    try:
        ...
        raise ValidationError(
            _("Invalid Postman configuration data: %(error)s") % {"error": str(e)}
        ) from e
