...
class UserRoleAccess(BaseModel):
    ...
    restricted_components: List[str] = Field(
        default_factory=list, description="Components this role cannot access"
    )
...
class AccessControl(BaseModel):
    ...
    super_admin: UserRoleAccess = Field(
        default_factory=lambda: UserRoleAccess(
...
class HookReference(BaseModel):
    ...
    dependencies: List[str] = Field(
        default_factory=list, description="Hook dependencies"
    )
...
class PageSections(BaseModel):
    ...
    headings: List[HeadingElement] = Field(
        default_factory=list, description="Heading elements"
    )
    subheadings: List[HeadingElement] = Field(
        default_factory=list, description="Subheading elements"
    )
    tabs: List[TabElement] = Field(default_factory=list, description="Tab elements")
    buttons: List[ButtonElement] = Field(
        default_factory=list, description="Button elements"
    )
    input_boxes: List[InputBoxElement] = Field(
        default_factory=list, description="Input box elements"
    )
    text_blocks: List[TextBlockElement] = Field(
        default_factory=list, description="Text block elements"
    )
    components: List[ComponentReference] = Field(
        default_factory=list, description="Component references"
    )
    utilities: List[UtilityReference] = Field(
        default_factory=list, description="Utility references"
    )
    services: List[ServiceReference] = Field(
        default_factory=list, description="Service references"
    )
    hooks: List[HookReference] = Field(
        default_factory=list, description="Hook references"
    )
    contexts: List[ContextReference] = Field(
        default_factory=list, description="Context references"
    )
    ui_components: List[ComponentReference] = Field(
        default_factory=list, description="UI component references"
    )
    endpoints: List[EndpointReferenceInSection] = Field(
        default_factory=list, description="Endpoint references"
    )
...
class PageEndpointUsage(BaseModel):
    ...
    endpoint_path: str = Field(
        ..., description="Endpoint path (e.g., 'graphql/GetUserStats')"
    )
    method: str = Field(
        ..., description="HTTP/GraphQL method (QUERY, MUTATION, GET, POST, etc.)"
    )
    ...
    via_hook: Optional[str] = Field(
        None, description="React hook name that uses this endpoint"
    )
    usage_type: str = Field(
        ..., description="Usage type: primary, secondary, or conditional"
    )
    usage_context: str = Field(
        ...,
        description="Usage context: data_fetching, data_mutation, authentication, or analytics",
    )
    description: Optional[str] = Field(
        None, description="Description of how this endpoint is used"
    )
...
class PageMetadata(BaseModel):
    ...
    s3_key: str = Field(
        ..., description="S3 key for JSON file (format: data/pages/{page_id}.json)"
    )
    status: str = Field(
        "published", description="Status: draft, published, archived, deleted"
    )
    authentication: str = Field(
        "Not required", description="Authentication requirement/notes"
    )
    authorization: Optional[str] = Field(
        None, description="Authorization requirement/notes"
    )
    page_state: str = Field(
        "development",
        description="Page state: coming_soon, published, draft, development, test",
    )
    last_updated: str = Field(..., description="Last updated timestamp (ISO 8601)")
    uses_endpoints: List[PageEndpointUsage] = Field(
        default_factory=list, description="List of endpoints used by this page"
    )
    ui_components: List[UIComponent] = Field(
        default_factory=list, description="List of UI components used by this page"
    )
    versions: List[str] = Field(
        default_factory=list, description="Page version history"
    )
    endpoint_count: int = Field(
        default=0,
        description="Number of endpoints used (should match uses_endpoints.length)",
    )
    api_versions: List[str] = Field(
        default_factory=list,
        description="List of API versions used (derived from uses_endpoints)",
    )
    content_sections: Optional[Dict[str, Any]] = Field(
        None, description="Content sections structure"
    )
...
class PageDocumentation(BaseModel):
    ...
    id: str = Field(
        ..., alias="_id", description="Unique identifier (format: '{page_id}-001')"
    )
    page_id: str = Field(..., description="Page identifier")
    page_type: str = Field(
        ..., description="Page type: dashboard, marketing, docs, product, or title"
    )
    metadata: PageMetadata = Field(..., description="Page metadata")
    content: Optional[str] = Field(
        None, description="Page content (markdown format, stored in JSON)"
    )
    ...
    fallback_data: List[DataReferenceOrDict] = Field(
        default_factory=list,
        description="Fallback data references (DataReference or extended dict)",
    )
    mock_data: List[DataReferenceOrDict] = Field(
        default_factory=list,
        description="Mock data references (DataReference or extended dict)",
    )
    demo_data: List[DataReferenceOrDict] = Field(
        default_factory=list,
        description="Demo data references (DataReference or extended dict)",
    )
...
class EndpointRoleAccess(BaseModel):
    ...
    restricted_fields: List[str] = Field(
        default_factory=list, description="Fields this role cannot access"
    )
...
class EndpointAccessControl(BaseModel):
    ...
    super_admin: EndpointRoleAccess = Field(
        default_factory=lambda: EndpointRoleAccess(can_access=True, can_execute=True)
    )
    admin: EndpointRoleAccess = Field(
        default_factory=lambda: EndpointRoleAccess(can_access=True, can_execute=True)
    )
    pro_user: EndpointRoleAccess = Field(
        default_factory=lambda: EndpointRoleAccess(
            can_access=True, can_execute=True, rate_limit="100/hour"
        )
    )
    free_user: EndpointRoleAccess = Field(
        default_factory=lambda: EndpointRoleAccess(
            can_access=True, can_execute=True, rate_limit="20/hour"
        )
    )
    guest: EndpointRoleAccess = Field(
        default_factory=lambda: EndpointRoleAccess(can_access=False, can_execute=False)
    )
...
class LambdaServices(BaseModel):
    ...
    primary: Optional[PrimaryLambdaService] = Field(
        None, description="Primary Lambda service"
    )
    dependencies: List[DependencyLambdaService] = Field(
        default_factory=list, description="Dependent Lambda services"
    )
...
class EndpointMethods(BaseModel):
    ...
    repository_methods: List[str] = Field(
        default_factory=list, description="Repository methods"
    )
    validation_methods: List[str] = Field(
        default_factory=list, description="Validation methods"
    )
    middleware_methods: List[str] = Field(
        default_factory=list, description="Middleware methods"
    )
...
class EndpointPageUsage(BaseModel):
    ...
    updated_at: Optional[str] = Field(
        None, description="Last updated timestamp (ISO 8601)"
    )
...
class EndpointDocumentation(BaseModel):
    ...
    model_config = ConfigDict(populate_by_name=True)
    id: str = Field(
        ..., alias="_id", description="Unique identifier (format: '{endpoint_id}-001')"
    )
    endpoint_id: str = Field(..., description="Endpoint identifier")
    endpoint_path: str = Field(
        ..., description="Endpoint path (e.g., 'graphql/Login' or 'Login')"
    )
    method: str = Field(..., description="HTTP/GraphQL method")
    api_version: str = Field(..., description="API version (e.g., 'graphql')")
    ...
    endpoint_state: str = Field(
        "development",
        description="Endpoint state: coming_soon, published, draft, development, test",
    )
    service_file: Optional[str] = Field(None, description="Service file path")
    router_file: Optional[str] = Field(None, description="Router file path")
    service_methods: List[str] = Field(
    ...
    used_by_pages: List[EndpointPageUsage] = Field(
        default_factory=list, description="List of pages using this endpoint"
    )
    rate_limit: Optional[str] = Field(None, description="Rate limit information")
    graphql_operation: Optional[str] = Field(
        None, description="GraphQL operation string"
    )
    sql_file: Optional[str] = Field(None, description="SQL file reference")
    page_count: int = Field(
        default=0,
        description="Number of pages using this endpoint (should match used_by_pages.length)",
    )
    access_control: Optional[EndpointAccessControl] = Field(
        None, description="Access control settings"
    )
    lambda_services: Optional[LambdaServices] = Field(
        None, description="Lambda services configuration"
    )
...
class RelationshipAccessControl(BaseModel):
    ...
    super_admin: RelationshipRoleAccess = Field(
        default_factory=lambda: RelationshipRoleAccess(
            can_view=True, can_edit=True, can_delete=True
        )
    )
    admin: RelationshipRoleAccess = Field(
        default_factory=lambda: RelationshipRoleAccess(
            can_view=True, can_edit=True, can_delete=False
        )
    )
    pro_user: RelationshipRoleAccess = Field(
        default_factory=lambda: RelationshipRoleAccess(
            can_view=True, can_edit=False, can_delete=False
        )
    )
    free_user: RelationshipRoleAccess = Field(
        default_factory=lambda: RelationshipRoleAccess(
            can_view=True, can_edit=False, can_delete=False
        )
    )
    guest: RelationshipRoleAccess = Field(
        default_factory=lambda: RelationshipRoleAccess(
            can_view=False, can_edit=False, can_delete=False
        )
    )
...
class EndpointRef(BaseModel):
    ...
    endpoint_state: str = Field(
        "development", description="Endpoint state (default when omitted)"
    )
...
class EnhancedRelationship(BaseModel):
    ...
    access_control: Optional[RelationshipAccessControl] = Field(
        None, description="Access control"
    )
    page_reference: Optional[PageReference] = Field(None, description="Page reference")
    endpoint_reference: Optional[EndpointRef] = Field(
        None, description="Endpoint reference"
    )
    connection: Optional[RelationshipConnection] = Field(
