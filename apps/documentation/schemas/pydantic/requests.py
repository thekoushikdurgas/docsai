...
class DocumentationPageCreateRequest(BaseModel):
    ...
    page_type: Optional[str] = Field(
        None, description="Page type: 'marketing', 'dashboard', or 'docs' (default)"
    )
...
class DocumentationPageUpdateRequest(BaseModel):
    ...
    metadata: Optional[PageMetadata] = Field(
        None, description="Updated metadata (partial updates supported)"
    )
...
class EndpointDocumentationCreateRequest(BaseModel):
    ...
    service_file: Optional[str] = Field(
        None,
        description="Service file path (at least one of service_file or router_file required)",
    )
    router_file: Optional[str] = Field(
        None,
        description="Router file path (at least one of service_file or router_file required)",
    )
    ...
    used_by_pages: Optional[List[Dict[str, Any]]] = Field(
        None, description="List of pages using this endpoint"
    )
    page_count: Optional[int] = Field(
        None, description="Number of pages (auto-calculated if not provided)"
    )
...
class RelationshipCreateRequest(BaseModel):
    ...
    usage_type: str = Field(
        default="primary", description="Usage type: primary, secondary, or conditional"
    )
