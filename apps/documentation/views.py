    ...
    try:
        ...
        if search_query:
            pages = [
                p
                for p in pages
                if search_query.lower() in (p.get("title", "") or "").lower()
                or search_query.lower() in (p.get("page_id", "") or "").lower()
            ]
...
def get_page_view(request, page_id):
    ...
    if isinstance(page, dict):
        page["content"] = (
            page.get("content") or (page.get("metadata") or {}).get("content", "") or ""
        )
    context = {
        "page": page,
    }
...
def delete_page_view(request, page_id):
    ...
    try:
        ...
        breadcrumb_items = [
            ...
            {
                "label": page.get("title") or page.get("page_id", "")[:30],
                "url": reverse(
                    "documentation:page_detail", kwargs={"page_id": page_id}
                ),
            },
            {"label": "Delete"},
        ]
        ...
        if page.get("relationships_count") or page.get("endpoints_count"):
            ...
            if page.get("relationships_count"):
                items.append(
                    f"• This page has {page['relationships_count']} relationship(s) that will be affected"
                )
            if page.get("endpoints_count"):
                items.append(
                    f"• This page references {page['endpoints_count']} endpoint(s)"
                )
