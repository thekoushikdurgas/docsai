...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        if not pages_dir.exists():
            self.stdout.write(
                self.style.ERROR(f"Pages directory not found: {pages_dir}")
            )
            return
        ...
        for file_path in files:
            ...
            if not match:
                ...
                if filename == "README.md":
                    continue
                self.stdout.write(
                    self.style.WARNING(f"Skipping invalid filename pattern: {filename}")
                )
                continue
        ...
        self.stdout.write(
            self.style.SUCCESS(f"Successfully imported {success_count} pages.")
        )
