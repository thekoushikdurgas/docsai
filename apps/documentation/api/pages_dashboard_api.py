...
def get_pages_list_api(request: HttpRequest) -> JsonResponse:
    ...
    try:
        ...
        if sort_field:
            ...
            if sort_field == "name":
                pages.sort(
                    key=lambda p: p.get("page_id", "").lower(), reverse=reverse_order
                )
        ...
        return error_response(
            message=f"Failed to retrieve pages: {str(e)}", status_code=500
        ).to_json_response()
...
def pages_bulk_import_check_api(request: HttpRequest) -> JsonResponse:
    ...
    if not isinstance(page_ids, list):
        return error_response(
            "page_ids must be a list.", status_code=400
        ).to_json_response()
...
def pages_bulk_import_preview_api(request: HttpRequest) -> JsonResponse:
    ...
    if not isinstance(pages, list) or not pages:
        return error_response(
            "Field 'pages' must be a non-empty list.", status_code=400
        ).to_json_response()
    if len(pages) > 500:
        return error_response(
            "Maximum 500 pages per import.", status_code=400
        ).to_json_response()
    ...
    try:
        ...
        logger.error(
            "pages_bulk_import_preview_api import failed: %s", e, exc_info=True
        )
    ...
    for index, raw_row in enumerate(pages, start=1):
        ...
        if not page_id:
            validation_errors.append(
                {"row": index, "page_id": "", "error": "page_id is required"}
            )
            continue
        try:
            page_data = (
                DataTransformer.lambda_to_django_page(row) if "_id" in row else row
            )
            ...
            validation_errors.append(
                {"row": index, "page_id": page_id, "error": str(exc)}
            )
...
def pages_bulk_import_api(request: HttpRequest) -> JsonResponse:
    ...
    try:
        ...
        logger.error(
            "Failed to import helpers for pages_bulk_import_api: %s", e, exc_info=True
        )
        return error_response(
...
def pages_bulk_upload_to_s3_api(request: HttpRequest) -> JsonResponse:
    ...
    if len(pages) > 500:
        return error_response(
            "Maximum 500 pages per upload.", status_code=400
        ).to_json_response()
    ...
    for row in pages:
        ...
        try:
            ...
            logger.warning(
                "pages_bulk_upload_to_s3_api page %s failed: %s", page_id, exc
            )
...
def pages_import_one_api(request: HttpRequest) -> JsonResponse:
    ...
    if not row or not isinstance(row, dict):
        return error_response(
            "Field 'page' must be a non-empty object.", status_code=400
        ).to_json_response()

    page_id = str(row.get("page_id") or "").strip()
    if not page_id:
        return error_response(
            "page_id is required.", status_code=400
        ).to_json_response()
    ...
    try:
        ...
        logger.error(
            "Failed to import helpers for pages_import_one_api: %s", e, exc_info=True
        )
        return error_response("Internal error.", status_code=500).to_json_response()

    service = get_pages_service()
    page_data = (
        DataTransformer.lambda_to_django_page(row) if "_id" in row else dict(row)
    )
    if not (page_data.get("title") or str(page_data.get("title", "")).strip()):
        metadata = page_data.get("metadata") or {}
        purpose = metadata.get("purpose", "")
    ...
    try:
        if existing:
            try:
                service.update_page(page_id, page_data)
                return success_response(
                    data={"updated": True, "page_id": page_id}, message="Page updated"
                ).to_json_response()
            except (ValueError, Exception) as upd_exc:
                if "Page not found" in str(upd_exc):
                    service.create_page(page_data)
                    return success_response(
                        data={"created": True, "page_id": page_id},
                        message="Page created",
                    ).to_json_response()
                raise upd_exc
        service.create_page(page_data)
        return success_response(
            data={"created": True, "page_id": page_id}, message="Page created"
        ).to_json_response()
    except Exception as exc:
