...
def pages_list_api(request):
    ...
    return JsonResponse(
        {"success": True, "items": items, "total": result.get("total", 0)}
    )
...
def pages_create_api(request):
    ...
    try:
        ...
        if page:
            return JsonResponse({"success": True, "data": page}, status=201)
        return JsonResponse(
            {"success": False, "error": "Failed to create page"}, status=400
        )
...
def pages_update_api(request, page_id):
    ...
    try:
        ...
        if page:
            return JsonResponse({"success": True, "data": page})
        return JsonResponse(
            {"success": False, "error": "Page not found or update failed"}, status=404
        )
    except json.JSONDecodeError:
        return JsonResponse({"success": False, "error": "Invalid JSON"}, status=400)
    except Exception as e:
...
def pages_delete_api(request, page_id):
    ...
    return JsonResponse(
        {"success": False, "error": "Page not found or delete failed"}, status=404
    )
...
def endpoints_create_api(request):
    ...
    try:
        ...
        if endpoint:
            return JsonResponse({"success": True, "data": endpoint}, status=201)
        return JsonResponse(
            {"success": False, "error": "Failed to create endpoint"}, status=400
        )
    except json.JSONDecodeError:
        return JsonResponse({"success": False, "error": "Invalid JSON"}, status=400)
...
def endpoints_update_api(request, endpoint_id):
    ...
    try:
        ...
        if endpoint:
            return JsonResponse({"success": True, "data": endpoint})
        return JsonResponse(
            {"success": False, "error": "Endpoint not found or update failed"},
            status=404,
        )
    except json.JSONDecodeError:
        return JsonResponse({"success": False, "error": "Invalid JSON"}, status=400)
...
def endpoints_delete_api(request, endpoint_id):
    ...
    if success:
        return JsonResponse({"success": True, "message": "Endpoint deleted"})
    return JsonResponse(
        {"success": False, "error": "Endpoint not found or delete failed"}, status=404
    )
...
def relationships_detail_api(request, relationship_id):
    ...
    if relationship:
        return JsonResponse({"success": True, "data": relationship})
    return JsonResponse(
        {"success": False, "error": "Relationship not found"}, status=404
    )
...
def relationships_create_api(request):
    ...
    try:
        ...
        if relationship:
            return JsonResponse({"success": True, "data": relationship}, status=201)
        return JsonResponse(
            {"success": False, "error": "Failed to create relationship"}, status=400
        )
    except json.JSONDecodeError:
        return JsonResponse({"success": False, "error": "Invalid JSON"}, status=400)
...
def relationships_delete_api(request, relationship_id):
    ...
    if success:
        return JsonResponse({"success": True, "message": "Relationship deleted"})
    return JsonResponse(
        {"success": False, "error": "Relationship not found or delete failed"},
        status=404,
    )
