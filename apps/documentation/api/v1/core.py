...
def pages_list(request: HttpRequest) -> JsonResponse:
    ...
    try:
        ...
        result = service.list_pages(
            page_type=page_type, status=status, limit=limit, offset=offset
        )
        ...
        return paginated_response(
            data=items, total=total, page=1, page_size=limit, message=""
        ).to_json_response()
...
def endpoints_list(request: HttpRequest) -> JsonResponse:
    ...
    try:
        ...
        result = service.list_endpoints(
            api_version=api_version, method=method, limit=limit, offset=offset
        )
...
def endpoints_detail(request: HttpRequest, endpoint_id: str) -> JsonResponse:
    ...
    try:
        ...
        if not endpoint:
            return JsonResponse(
                {"success": False, "error": "Endpoint not found"}, status=404
            )

        return JsonResponse({"success": True, "data": endpoint})
    except Exception as e:
...
def postman_list(request: HttpRequest) -> JsonResponse:
    ...
    try:
        ...
        result = service.list_configurations(state=state, limit=limit, offset=offset)
    ...
    try:
        ...
        if not config:
            return not_found_response(
                resource="Postman configuration"
            ).to_json_response()
    ...
    try:
        ...
        if search:
            ...
            items = [
                item
                for item in items
                if search_lower in str(item.get("page_id", "")).lower()
                or search_lower
                in str(item.get("metadata", {}).get("title", "")).lower()
            ]
        ...
        total = result.get("total", 0)
        return paginated_response(
            data=items, total=total, page=page, page_size=page_size, message=""
        ).to_json_response()
    except Exception as e:
...
def dashboard_endpoints(request: HttpRequest) -> JsonResponse:
    ...
    try:
        ...
        result = service.list_endpoints(
            api_version=api_version, method=method, limit=page_size, offset=offset
        )
        ...
        if search:
            ...
            items = [
                item
                for item in items
                if search_lower in str(item.get("endpoint_id", "")).lower()
                or search_lower in str(item.get("endpoint_path", "")).lower()
            ]
        ...
        return JsonResponse(
            {
                ...
                "pagination": {
                    ...
                    "page": page,
                    "page_size": page_size,
                    "total_pages": (result.get("total", 0) + page_size - 1)
                    // page_size,
                    "has_previous": page > 1,
...
def dashboard_relationships(request: HttpRequest) -> JsonResponse:
    ...
    try:
        ...
        result = service.list_relationships(
            page_id=page_id, endpoint_id=endpoint_id, limit=page_size, offset=offset
        )
        ...
        if search:
            ...
            items = [
                item
                for item in items
                if search_lower in str(item.get("relationship_id", "")).lower()
                or search_lower in str(item.get("page_path", "")).lower()
                or search_lower in str(item.get("endpoint_path", "")).lower()
            ]
    ...
    try:
        ...
        if search:
            ...
            items = [
                item
                for item in items
                if search_lower in str(item.get("config_id", "")).lower()
...
def endpoints_methods(request: HttpRequest) -> JsonResponse:
    ...
    try:
        ...
        return success_response(data={"methods": [], "total": 0}).to_json_response()
...
def endpoints_api_versions(request: HttpRequest) -> JsonResponse:
    ...
    try:
        ...
        versions = {}
        for ep in endpoints:
            api_version = ep.get("api_version") or ep.get("metadata", {}).get(
                "api_version", "unknown"
            )
...
def pages_types(request: HttpRequest) -> JsonResponse:
    ...
    try:
        ...
        types = {}
        for page in pages:
            page_type = page.get("metadata", {}).get("page_type") or page.get(
                "page_type", "docs"
            )
        ...
        return success_response(data={"types": [], "total": 0}).to_json_response()
...
def pages_statistics(request: HttpRequest) -> JsonResponse:
    ...
    try:
                ...
                "statistics": {
                    ...
                    "published": len(
                        [
                            p
                            for p in pages
                            if p.get("metadata", {}).get("status") == "published"
                        ]
                    ),
                    "draft": len(
                        [
                            p
                            for p in pages
                            if p.get("metadata", {}).get("status") == "draft"
                        ]
                    ),
                },
            }
        ...
        return success_response(data={"total": 0, "statistics": {}}).to_json_response()
    ...
    try:
        ...
        return success_response(
            data={
                ...
                "statistics": {"total_configurations": len(configurations)},
            }
        ).to_json_response()
    except Exception as e:
