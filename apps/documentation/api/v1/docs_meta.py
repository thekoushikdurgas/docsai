...
def endpoint_stats_by_user_type(request: HttpRequest) -> JsonResponse:
    ...
    try:
        ...
        if user_type_filter:
            ...
            for endpoint_key, user_types in by_endpoint.items():
                if user_type_filter in user_types:
                    filtered[endpoint_key] = {
                        user_type_filter: user_types[user_type_filter]
                    }
        ...
        # Calculate summary
        total_requests = sum(
            ut.get("total_requests", 0) for ut in by_user_type.values()
        )
        user_types_active = sum(
            1 for ut in by_user_type.values() if ut.get("total_requests", 0) > 0
        )
        ...
        if response_format == "graph":
            ...
            for endpoint_key, user_types in by_endpoint.items():
                ...
                for user_type in graph_data["user_types"]:
                    endpoint_data[user_type] = user_types.get(user_type, {}).get(
                        "request_count", 0
                    )
