...
def _by_api_version_list(request: HttpRequest, api_version: str) -> JsonResponse:
    try:
        service = get_adapter(request)
...
def _by_method_list(request: HttpRequest, method: str) -> JsonResponse:
    try:
        service = get_adapter(request)
