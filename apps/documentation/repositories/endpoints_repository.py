...
class EndpointsRepository(BaseRepository):
    ...
    def __init__(
        ...
        super().__init__(
            resource_name="endpoints", storage=storage, index_manager=index_manager
        )
    ...
    def get_by_path_and_method(
        ...
        for ep in endpoints_list:
            if (
                ep.get("path") == endpoint_path
                or ep.get("endpoint_path") == endpoint_path
            ):
    ...
    def get_endpoints_by_version_and_method(
        ...
        return self.list_all(
            api_version=api_version, method=method, limit=None, offset=0
        )
    ...
    def get_api_version_statistics(self) -> Dict[str, Any]:
        ...
        by_version = index_data.get("indexes", {}).get("by_api_version", {})
        statistics = [
            {"api_version": v, "count": len(ids)} for v, ids in by_version.items()
        ]
    ...
    def create(self, endpoint_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            self.logger.error(
                f"Validation error for endpoint {endpoint_id}: {e}", exc_info=True
            )
        ...
        try:
            ...
            try:
                self.index_manager.add_item_to_index(
                    "endpoints", endpoint_id, endpoint_data
                )
                self.logger.debug(
                    f"Incrementally updated index for endpoint: {endpoint_id}"
                )
            except Exception as e:
                self.logger.warning(
                    f"Failed to update index for endpoint {endpoint_id}: {e}"
                )
    ...
    def delete(self, endpoint_id: str) -> bool:
        ...
        try:
            ...
            try:
                ...
                self.logger.debug(
                    f"Incrementally removed endpoint from index: {endpoint_id}"
                )
            except Exception as e:
                self.logger.warning(
                    f"Failed to remove endpoint from index {endpoint_id}: {e}"
                )
    ...
    def transaction(self):
        ...
        class TransactionContext:
            ...
            def update(
                ...
                self.ops.append(
                    {"type": "update", "endpoint_id": endpoint_id, "result": result}
                )
                return result
            ...
            def delete(self, endpoint_id: str) -> bool:
                ...
                self.ops.append(
                    {"type": "delete", "endpoint_id": endpoint_id, "result": result}
                )
                return result
        ...
        try:
            ...
            for op in operations:
                self.logger.warning(
                    f"Transaction operation: {op['type']} on {op.get('endpoint_id', 'unknown')}"
                )
            raise
    ...
    def batch_create(
        ...
        for endpoint_data in endpoints_data:
            try:
                ...
                self.logger.error(
                    f"Failed to create endpoint {endpoint_id} in batch: {e}"
                )

        if errors:
            error_msg = (
                f"Batch create failed for {len(errors)}/{len(endpoints_data)} endpoints"
            )
    ...
    def batch_update(
        ...
        for endpoint_id, endpoint_data in updates:
            try:
                ...
                errors.append((endpoint_id, e))
                self.logger.error(
                    f"Failed to update endpoint {endpoint_id} in batch: {e}"
                )

        if errors:
            error_msg = (
                f"Batch update failed for {len(errors)}/{len(updates)} endpoints"
            )
            self.logger.error(error_msg)
            raise RepositoryError(
                message=error_msg,
    ...
    def batch_delete(self, endpoint_ids: List[str]) -> List[str]:
        ...
        for endpoint_id in endpoint_ids:
            try:
                ...
                errors.append((endpoint_id, e))
                self.logger.error(
                    f"Failed to delete endpoint {endpoint_id} in batch: {e}"
                )

        if errors:
            error_msg = (
                f"Batch delete failed for {len(errors)}/{len(endpoint_ids)} endpoints"
            )
            self.logger.error(error_msg)
            raise RepositoryError(
                message=error_msg,
