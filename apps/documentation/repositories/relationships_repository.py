...
class RelationshipsRepository(BaseRepository):
    ...
    def __init__(
        ...
        super().__init__(
            resource_name="relationships", storage=storage, index_manager=index_manager
        )
    ...
    def list_all(
        ...
        try:
            ...
            if page_id or endpoint_id:
                ...
                if endpoint_id:
                    ...
                    for endpoint_key, rel_ids in by_endpoint_index.items():
                        if endpoint_id in endpoint_key or endpoint_key.endswith(
                            f":{endpoint_id}"
                        ):
                            relationship_ids.update(rel_ids)
            ...
            if not relationships_list:
                self.logger.debug(
                    "Index empty or not available, falling back to file scanning"
                )
                relationships_list = self._scan_relationships_files(
                    page_id, endpoint_id
                )
            ...
            self.logger.debug(
                f"Listed {len(result_relationships)} relationships (filtered: page_id={page_id}, endpoint_id={endpoint_id})"
            )
            ...
            try:
                return self._scan_relationships_files(
                    page_id, endpoint_id, limit, offset
                )
            except Exception as fallback_error:
                self.logger.error(
                    f"Fallback file scanning also failed: {fallback_error}",
                    exc_info=True,
                )
    ...
    def _scan_relationships_files(
        ...
        try:
            ...
            file_keys = self.storage.list_json_files(
                self.relationships_prefix, max_keys=10000
            )
            ...
            for file_key in json_files:
                try:
                    ...
                    # Apply filters
                    if page_id:
                        rel_page_id = relationship_data.get(
                            "page_id"
                        ) or relationship_data.get("page_path")
                        if not (
                            page_id in str(rel_page_id) or str(rel_page_id) in page_id
                        ):
                            continue

                    if endpoint_id:
                        rel_endpoint_id = relationship_data.get(
                            "endpoint_id"
                        ) or relationship_data.get("endpoint_path")
                        if not (
                            endpoint_id in str(rel_endpoint_id)
                            or str(rel_endpoint_id) in endpoint_id
                        ):
                            continue

                    # Ensure _id is set
                    relationship_id = relationship_data.get(
                        "relationship_id"
                    ) or file_key.split("/")[-1].replace(".json", "")
                    ...
                    self.logger.warning(
                        f"Failed to read relationship file {file_key}: {e}"
                    )
                    continue
    ...
    def create(self, relationship_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            try:
                self.index_manager.add_item_to_index(
                    "relationships", relationship_id, relationship_data
                )
                self.logger.debug(
                    f"Incrementally updated index for relationship: {relationship_id}"
                )
            except Exception as e:
                self.logger.warning(
                    f"Failed to update index for relationship {relationship_id}: {e}"
                )
    ...
    def update(
        ...
        try:
            ...
            try:
                self.index_manager.add_item_to_index(
                    "relationships", relationship_id, validated
                )
                self.logger.debug(
                    f"Incrementally updated index for relationship: {relationship_id}"
                )
    ...
    def delete(self, relationship_id: str) -> bool:
        ...
        try:
            ...
            # Incrementally update index
            try:
                self.index_manager.remove_item_from_index(
                    "relationships", relationship_id
                )
                self.logger.debug(
                    f"Incrementally removed relationship from index: {relationship_id}"
                )
            except Exception as e:
                self.logger.warning(
                    f"Failed to remove relationship from index {relationship_id}: {e}"
                )
    ...
    def transaction(self):
        ...
        class TransactionContext:
            def __init__(
                self, repo: "RelationshipsRepository", ops: List[Dict[str, Any]]
            ):
                self.repo = repo
                self.ops = ops
        ...
        try:
            ...
            for op in operations:
                self.logger.warning(
                    f"Transaction operation: {op['type']} on {op.get('relationship_id', 'unknown')}"
                )
            raise
    ...
    def batch_create(
        ...
        for relationship_data in relationships_data:
            try:
                ...
                self.logger.error(
                    f"Failed to create relationship {relationship_id} in batch: {e}"
                )
    ...
    def batch_update(
        ...
        for relationship_id, relationship_data in updates:
            try:
                ...
                errors.append((relationship_id, e))
                self.logger.error(
                    f"Failed to update relationship {relationship_id} in batch: {e}"
                )

        if errors:
            error_msg = (
                f"Batch update failed for {len(errors)}/{len(updates)} relationships"
            )
            self.logger.error(error_msg)
            raise RepositoryError(
                message=error_msg,
    ...
    def batch_delete(self, relationship_ids: List[str]) -> List[str]:
        ...
        for relationship_id in relationship_ids:
            try:
                ...
                errors.append((relationship_id, e))
                self.logger.error(
                    f"Failed to delete relationship {relationship_id} in batch: {e}"
                )
