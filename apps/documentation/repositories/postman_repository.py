...
class PostmanRepository:
    ...
    def list_collections(
        ...
        try:
            ...
            for coll_summary in paginated_collections:
                collection_id = coll_summary.get("collection_id") or coll_summary.get(
                    "id"
                )
    ...
    def list_environments(
        ...
        try:
            ...
            for env_summary in paginated_environments:
                environment_id = env_summary.get("environment_id") or env_summary.get(
                    "id"
                )
                if not environment_id:
                    continue
        ...
        try:
            ...
            for file_key in json_files:
                try:
                    ...
                    collection_id = collection_data.get("id") or collection_data.get(
                        "collection_id"
                    )
                    if not collection_id:
                    ...
                    self.logger.warning(
                        f"Failed to read collection file {file_key}: {e}"
                    )
                    continue
        ...
        try:
            ...
            for file_key in json_files:
                try:
                    ...
                    environment_id = environment_data.get("id") or environment_data.get(
                        "environment_id"
                    )
                    if not environment_id:
                    ...
                    self.logger.warning(
                        f"Failed to read environment file {file_key}: {e}"
                    )
                    continue
    ...
    def transaction(self):
        ...
        try:
            ...
            for op in operations:
                self.logger.warning(
                    f"Transaction operation: {op['type']} on {op.get('collection_id') or op.get('environment_id', 'unknown')}"
                )
            raise

    def batch_create_collections(
        self, collections_data: List[Dict[str, Any]]
    ) -> List[Dict[str, Any]]:
        """
        ...
        for collection_data in collections_data:
            try:
                ...
                self.logger.error(
                    f"Failed to create collection {collection_id} in batch: {e}"
                )
    ...
    def batch_update_collections(
        ...
        for collection_id, collection_data in updates:
            try:
                ...
                errors.append((collection_id, e))
                self.logger.error(
                    f"Failed to update collection {collection_id} in batch: {e}"
                )

        if errors:
            error_msg = (
                f"Batch update failed for {len(errors)}/{len(updates)} collections"
            )
            self.logger.error(error_msg)
            raise RepositoryError(
                message=error_msg,
    ...
    def batch_delete_collections(self, collection_ids: List[str]) -> List[str]:
        ...
        for collection_id in collection_ids:
            try:
                ...
                errors.append((collection_id, e))
                self.logger.error(
                    f"Failed to delete collection {collection_id} in batch: {e}"
                )
    ...
    def batch_create_environments(
        self, environments_data: List[Dict[str, Any]]
    ) -> List[Dict[str, Any]]:
        """
        ...
        for environment_data in environments_data:
            try:
                ...
                self.logger.error(
                    f"Failed to create environment {environment_id} in batch: {e}"
                )
    ...
    def batch_update_environments(
        ...
        for environment_id, environment_data in updates:
            try:
                ...
                errors.append((environment_id, e))
                self.logger.error(
                    f"Failed to update environment {environment_id} in batch: {e}"
                )

        if errors:
            error_msg = (
                f"Batch update failed for {len(errors)}/{len(updates)} environments"
            )
            self.logger.error(error_msg)
            raise RepositoryError(
                message=error_msg,
    ...
    def batch_delete_environments(self, environment_ids: List[str]) -> List[str]:
        ...
        for environment_id in environment_ids:
            try:
                ...
                errors.append((environment_id, e))
                self.logger.error(
                    f"Failed to delete environment {environment_id} in batch: {e}"
                )
