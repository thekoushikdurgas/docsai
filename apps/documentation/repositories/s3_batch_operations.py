...
class S3BatchOperations:
    ...
    def batch_read_json(
        self, s3_keys: List[str], fail_on_error: bool = False
    ) -> Dict[str, Any]:
        """
        ...
        with ThreadPoolExecutor(max_workers=self.max_workers) as executor:
            future_to_key = {executor.submit(read_single, key): key for key in s3_keys}
    ...
    def batch_write_json(
        self, items: List[Tuple[str, Dict[str, Any]]], fail_on_error: bool = False
    ) -> Dict[str, bool]:
        """
        ...
        with ThreadPoolExecutor(max_workers=self.max_workers) as executor:
            future_to_key = {
                executor.submit(write_single, key, data): key for key, data in items
            }
    ...
    def batch_delete(
        ...
        with ThreadPoolExecutor(max_workers=self.max_workers) as executor:
            future_to_key = {
                executor.submit(delete_single, key): key for key in s3_keys
            }
