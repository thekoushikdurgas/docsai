...
class BaseUnifiedStorage:
    ...
    def __init__(
        ...
        self.enable_request_deduplication = getattr(
            settings, "ENABLE_REQUEST_DEDUPLICATION", True
        )
        self.enable_caching = getattr(settings, "UNIFIED_STORAGE_ENABLE_CACHE", True)
        self.fallback_enabled = getattr(
            settings, "UNIFIED_STORAGE_FALLBACK_ENABLED", True
        )
        ...
        try:
            ...
            self.s3_circuit_breaker = CircuitBreaker(
                "s3_storage", failure_threshold=5, timeout_seconds=60
            )
            if self.graphql_service:
                self.graphql_circuit_breaker = CircuitBreaker(
                    "graphql_api", failure_threshold=5, timeout_seconds=60
                )
            else:
        ...
        self.logger.debug(
            f"BaseUnifiedStorage initialized (caching={self.enable_caching}, fallback={self.fallback_enabled})"
        )

    def _get_cache_key(
        self, resource_type: str, identifier: str = None, filters: Optional[Dict] = None
    ) -> str:
        if identifier:
    ...
    def _track_backend_usage(self, backend: StorageBackend, success: bool = True):
        ...
        if success:
            self._metrics["backend_usage"][backend_name] = (
                self._metrics["backend_usage"].get(backend_name, 0) + 1
            )
        else:
            self._metrics["errors"][backend_name] = (
                self._metrics["errors"].get(backend_name, 0) + 1
            )

    def _handle_backend_error(
        self, backend: StorageBackend, error: Exception, operation: str
    ) -> None:
        self._track_backend_usage(backend, success=False)
        self.logger.warning(
            f"Backend {backend.value} failed for operation {operation}: {error}",
            exc_info=True,
        )
    ...
    def clear_cache(
        ...
        try:
            ...
            if resource_type and identifier:
                ...
                self.logger.debug(
                    f"Cleared cache for {resource_type}:{identifier} (deleted: {deleted})"
                )
            elif resource_type:
                pattern = f"unified_storage:{resource_type}:*"
                deleted = cache_manager.delete_pattern(pattern)
                self.logger.debug(
                    f"Cleared cache for resource type '{resource_type}' (deleted {deleted} keys)"
                )
            else:
                pattern = "unified_storage:*"
                deleted = cache_manager.delete_pattern(pattern)
                self.logger.debug(
                    f"Cleared all unified_storage cache (deleted {deleted} keys)"
                )
        except ImportError:
            if resource_type and identifier:
                cache_key = self._get_cache_key(resource_type, identifier)
    ...
    def get_metrics(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "circuit_breakers": {
                "s3": self.s3_circuit_breaker.get_status()
                if self.s3_circuit_breaker
                else None,
                "graphql": self.graphql_circuit_breaker.get_status()
                if self.graphql_circuit_breaker
                else None,
            },
        }
