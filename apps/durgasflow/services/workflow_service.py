...
class WorkflowService:
    ...
    def create_workflow(
        ...
        credential_mappings: Optional[Dict[str, Dict[str, Optional[str]]]] = None,
    ) -> Dict[str, Any]:
        """
        ...
        logger.debug(f"Created workflow: {workflow.get('id')} - {workflow.get('name')}")
        return workflow
    ...
    def save_graph(cls, workflow_id: str, graph_data: Dict) -> Optional[Dict[str, Any]]:
        ...
        if updated:
            logger.debug(f"Saved graph for workflow: {workflow_id}")
        return updated
    ...
    def duplicate_workflow(cls, workflow_id: str, user_uuid: str) -> Dict[str, Any]:
        ...
        logger.debug(f"Duplicated workflow {workflow_id} to {new_workflow.get('id')}")
        return new_workflow
    ...
    def import_n8n_workflow(cls, n8n_data: Dict, user_uuid: str) -> Dict[str, Any]:
        ...
        # Extract credential requirements for post-import mapping
        credential_mappings = N8nParser.extract_credential_requirements(n8n_data)
        ...
        logger.debug(f"Successfully imported n8n workflow: {workflow.get('id')} - {stats['supported_nodes']}/{stats['total_nodes']} nodes converted")
        return workflow
