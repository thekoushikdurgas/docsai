...
class ManualTriggerNode(BaseNodeHandler):
    ...
    def execute(self, config: Dict, input_data: Any, context: Any) -> Any:
        return {
            'triggered_at': str(context.execution.get('started_at', '')),
            'trigger_data': context.trigger_data,
            'user': context.execution.get('triggered_by') or None  # Appointment360 user UUID string
        }
...
class ScheduleTriggerNode(BaseNodeHandler):
    ...
    def execute(self, config: Dict, input_data: Any, context: Any) -> Any:
        return {
            'scheduled_at': str(context.execution.get('started_at', '')),
