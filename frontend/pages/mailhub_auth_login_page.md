---
title: "Mailhub login"
page_id: mailhub_auth_login_page
source_json: mailhub_auth_login_page.json
...
- **flow_id:** mailhub-auth-login
- **_id:** mailhub_auth_login_page-001
...
- **route:** /auth/login
- **file_path:** contact360.io/email/src/app/auth/login/page.tsx
- **purpose:** Mailhub auth login (separate from app.contact360.io login).
- **s3_key:** data/pages/mailhub_auth_login_page.json
- **status:** published
...
Authentication for Mailhub client.
...
**Composite layout:** [L:Auth] > [Q:AuthForm]
...
**Route (registry):** `/auth/login`

**Codebase:** `contact360.io/email` (Mailhub client).

**Typical inbound:** Browser direct entry; [mailhub_auth_signup_page.md](mailhub_auth_signup_page.md) login link.

**Typical outbound:** [mailhub_home_page.md](mailhub_home_page.md) on success.

**Cross-host:** Hand-off to `root` (Marketing) or `app` (Dashboard) via Connectra SSO.
