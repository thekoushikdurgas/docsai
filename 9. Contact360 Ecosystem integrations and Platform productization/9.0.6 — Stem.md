...
## Tasks
### Surface
- 📌 Planned: **jobs**: shape v9.0 surface outcomes for tenant config overlays; surface job lifecycle visibility for operators in `contact360.io/jobs` while advancing packaging/runtime plans.
- 📌 Planned: **mailvetter**: shape v9.0 surface outcomes for tenant config overlays; present verifier rationale fields for audit readability in `backend(dev)/mailvetter` while advancing tenant config overlays.
...
### Data
- 📌 Planned: **jobs**: anchor v9.0 data outcomes for tenant config overlays; record queue attempt history with reproducible markers in `contact360.io/jobs` while advancing packaging/runtime plans.
- 📌 Planned: **mailvetter**: anchor v9.0 data outcomes for tenant config overlays; store verdict evidence artifacts with replay metadata in `backend(dev)/mailvetter` while advancing tenant config overlays.
