...
## Tasks
### Contract
- 📌 Planned: **api**: define v9.0 contract outcomes for tenant config overlays; harden request/response schema boundaries in `contact360.io/api` while advancing entitlement enforcement.
- 📌 Planned: **admin**: define v9.0 contract outcomes for tenant config overlays; formalize control-plane request contracts and guardrails in `contact360.io/admin` while advancing tenant config overlays.
- 📌 Planned: Define entitlement-aware VQL policy contract for tenant plans in `app/services/query/*`.
- `POST /common/jobs/create`

### Service
- 📌 Planned: **api**: deliver v9.0 service outcomes for tenant config overlays; implement strict handler guards and deterministic branching in `contact360.io/api` while advancing entitlement enforcement.
- 📌 Planned: **admin**: deliver v9.0 service outcomes for tenant config overlays; harden operator workflows and privilege-aware actions in `contact360.io/admin` while advancing tenant config overlays.
...
- 📌 Planned: **[app]** — Verify UX for route `/` and bindings (patch 9.0.0 band 0) | area: `frontend-page` | files: `contact360/dashboard/app/page.tsx` | reason: Dashboard/extension surface for era 9 must match gateway contracts
...
- 📌 Planned: **[platform]** — Record smoke evidence, rollback, and alerts (patch band 0: charter/P0) | area: `ops` | files: `docs/commands/`, `.github/workflows/` | reason: Smoke, rollback, and observability for patch 9.0.0
