...
- No dashboard MV3/UI change in this patch; satellite serves presigned flows consumed by `contact360.io/app` and workers only.

### Data

- Queue durability for metadata jobs: Postgres `s3storage_metadata_jobs` via `internal/pgqueue`; multipart session TTL sweep limits orphan state in memory.
