...
(function () {
  ...
  function buildSubColumn(nestedBranchLi) {
    ...
    children.forEach(function (li) {
      if (li.classList.contains('nav-leaf')) {
        ...
        a.textContent = link.querySelector('span') ? link.querySelector('span').textContent.trim() : link.textContent.trim();
        ...
        var label = toggle
          ? (toggle.querySelector('.nav-branch-label') || toggle).textContent.trim()
          : '';
        ...
        if (inner) {
          inner.querySelectorAll(':scope > li.nav-leaf').forEach(function (leaf) {
    ...
    items.forEach(function (li) {
      if (li.classList.contains('nav-leaf')) {
        ...
        row.addEventListener('mouseenter', function () {
          primary.querySelectorAll('.c360-admin-flyout__row').forEach(function (r) {
  ...
  document.addEventListener(
    ...
    function (e) {
      ...
      if (!branchLi.id) {
        branchLi.id = 'nav-branch-flyout-' + Math.random().toString(36).slice(2);
      }
