...
(function () {
  ...
  var UploadZone = {
    init: function (opts) {
      ...
      var progWrap = document.getElementById(opts.progressWrapId || 'pb-progress-wrap');
      var progBar = document.getElementById(opts.progressBarId || 'pb-progress-bar');
      ...
      function pick(f) {
        if (!f.name.toLowerCase().endsWith('.json')) {
          if (typeof showToast === 'function') showToast('Select a .json file.', 'error');
          return;
        }
        file = f;
      ...
      if (btn)
        btn.addEventListener('click', function () {
          ...
          if (bucket && bucket.value.trim()) fd.append('bucket_id', bucket.value.trim());
          fetch(url, {
            method: 'POST',
            ...
            .then(function (data) {
              ...
              if (data.success) {
                if (typeof showToast === 'function')
                  showToast('Uploaded: ' + (data.title || data.page_id), 'success');
                if (opts.onSuccess) {
                  opts.onSuccess(data);
                } else {
                ...
                if (typeof showToast === 'function') showToast(data.error || 'Upload failed', 'error');
                btn.disabled = false;
              }
            })
            .catch(function (e) {
  ...
  var EndpointsPanel = {
    render: function (spec, containerId) {
      ...
      var rows = eps
        .map(function (e) {
          var method = (e.method || e.operation || '—').toString().toUpperCase();
          var path = e.path || e.graphql_operation || e.operation || e.url || '—';
  ...
  var SectionsEditor = {
    ...
    render: function (sections, containerId) {
      ...
      if (!el) return;
      this._sections = Array.isArray(sections) ? JSON.parse(JSON.stringify(sections)) : [];
      el.innerHTML = '';
      this._sections.forEach(function (s, i) {
        ...
        head.innerHTML =
          '<i class="lni lni-chevron-right pb-chevron"></i><span>' + escHtml(h) + '</span>';
  ...
  var EditorApp = {
    init: function (opts) {
      ...
      PageSpecLoader.load(specId, jsonUrl, function (spec) {
        document.getElementById('pb-editor-title').textContent = spec.title || spec.page_id || '';
        ...
        saveBtn.addEventListener('click', function () {
          ...
          fetch(saveUrl, {
            ...
            .then(function (data) {
              ...
              if (data.success) {
                if (typeof showToast === 'function') showToast('Sections saved.', 'success');
              } else {
