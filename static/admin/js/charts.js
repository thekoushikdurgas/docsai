...
document.addEventListener("DOMContentLoaded", function () {
  document
    .querySelectorAll("canvas[data-chart-type]")
    .forEach(function (canvas) {
      ...
      if (type === "bar")
        window.initBarChart(canvas.id, data.labels, data.values, data.label);
      else if (type === "line")
        window.initLineChart(canvas.id, data.labels, data.values, data.label);
      else if (type === "doughnut")
        window.initDoughnutChart(canvas.id, data.labels, data.values);
    });
});
