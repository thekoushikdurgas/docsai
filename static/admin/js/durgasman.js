...
(function () {
  ...
  var EnvManager = (function () {
    ...
    function loadList() {
      return fetch("/durgasman/environments/")
        .then(function (r) {
          return r.json();
        })
        .then(function (data) {
    ...
    function _loadEnv(id) {
      fetch("/durgasman/environments/" + id + "/json/")
        .then(function (r) {
          return r.json();
        })
        .then(function (data) {
        ...
        .catch(function (e) {
          console.warn("EnvManager load failed:", e);
        });
    }
    ...
    function _renderVars() {
      ...
      if (!enabled.length) {
        _varTableEl.innerHTML =
          '<tr><td colspan="2" style="color:var(--c360-text-muted);padding:8px;font-size:var(--c360-text-xs)">No variables.</td></tr>';
        return;
      }
      _varTableEl.innerHTML = enabled
        .map(function (v) {
          return (
            "<tr>" +
            '<td class="pm-var-key">{{' +
            escHtml(v.key) +
            "}}</td>" +
            '<td><input class="pm-var-val" data-key="' +
            escHtml(v.key) +
            '" value="' +
            escHtml(v.value || "") +
            '"></td>' +
      ...
      _varTableEl.querySelectorAll(".pm-var-val").forEach(function (inp) {
        inp.addEventListener("change", function () {
          ...
          var val = _activeEnv.values.find(function (v) {
            return v.key === key;
          });
  ...
  var CollectionTree = (function () {
    ...
    function loadList() {
      return fetch("/durgasman/collections/")
        .then(function (r) {
          return r.json();
        })
        .then(function (data) {
    ...
    function _renderCollections() {
      ...
      if (!_collections.length) {
        _containerEl.innerHTML =
          '<div style="padding:var(--c360-spacing-4);font-size:var(--c360-text-xs);color:var(--c360-text-muted)">No collections. <a href="/durgasman/upload/" style="color:var(--c360-primary)">Upload one</a>.</div>';
        return;
      }
      _containerEl.innerHTML = "";
      _collections.forEach(function (col) {
        ...
        header.innerHTML =
          '<i class="lni lni-chevron-right pm-chevron"></i>' +
          '<i class="lni lni-folder pm-folder-icon"></i>' +
          '<span style="flex:1;overflow:hidden;text-overflow:ellipsis">' +
          escHtml(col.name) +
          "</span>" +
          '<span style="font-size:10px;color:var(--c360-text-muted);margin-left:4px">' +
          (col.request_count || 0) +
          "</span>";
    ...
    function _loadCollection(colId, childrenEl) {
      childrenEl.innerHTML =
        '<li style="padding:4px 16px;font-size:var(--c360-text-xs);color:var(--c360-text-muted)"><span class="pm-spinner"></span> Loading…</li>';
      fetch("/durgasman/collections/" + colId + "/json/")
        .then(function (r) {
          return r.json();
        })
        .then(function (data) {
          ...
          if (!data.success) {
            childrenEl.innerHTML =
              '<li style="padding:4px 16px;color:var(--c360-danger);font-size:var(--c360-text-xs)">Error: ' +
              escHtml(data.error) +
              "</li>";
            return;
          }
        ...
        .catch(function (e) {
          childrenEl.innerHTML =
            '<li style="padding:4px 16px;color:var(--c360-danger);font-size:var(--c360-text-xs)">' +
            escHtml(String(e)) +
            "</li>";
        });
    }
    ...
    function _renderItems(items, parentEl, depth) {
      items.forEach(function (item) {
        ...
        if (Array.isArray(item.item)) {
          ...
          folderEl.innerHTML =
            '<i class="lni lni-chevron-right pm-chevron"></i><i class="lni lni-folder pm-folder-icon"></i><span>' +
            escHtml(item.name) +
            "</span>";
          ...
          reqEl.innerHTML =
            '<span class="pm-method-pill pm-method-' +
            method +
            '">' +
            escHtml(method) +
            "</span>" +
            '<span class="pm-request-name">' +
            escHtml(item.name) +
            "</span>";
          reqEl.addEventListener("click", function () {
            document
              .querySelectorAll(".pm-tree-request")
              .forEach(function (el) {
    ...
    function filterTree(query) {
      ...
      document.querySelectorAll(".pm-tree-request").forEach(function (el) {
        var name =
          (el.querySelector(".pm-request-name") || {}).textContent || "";
  ...
  var RequestEditor = (function () {
    ...
    function init(opts) {
      ...
      radios.forEach(function (r) {
        r.addEventListener("change", function () {
          _toggleBodyType(r.value);
        });
      });
      if (_authType) {
        _authType.addEventListener("change", function () {
          _toggleAuthFields(_authType.value);
        });
      }
    }
    ...
    function loadRequest(item) {
      ...
      if (req.auth) {
        ...
        if (authType === "bearer" && req.auth.bearer && _authToken) {
          ...
          var tokenEntry = bearerArr.find(function (b) {
            return b.key === "token";
          });
          if (tokenEntry && _authToken)
            _authToken.value = tokenEntry.value || "";
        }
        if (authType === "basic" && req.auth.basic) {
          var basicArr = req.auth.basic;
          var userEntry = basicArr.find(function (b) {
            return b.key === "username";
          });
          var passEntry = basicArr.find(function (b) {
            return b.key === "password";
          });
    ...
    function _appendKVRow(table, key, val, disabled, keyPh, valPh) {
      ...
      tr.innerHTML =
        ...
        (disabled ? "" : "checked") +
        "></td>" +
        '<td style="width:40%"><input class="pm-kv-key" placeholder="' +
        keyPh +
        '" value="' +
        escHtml(key) +
        '"></td>' +
        '<td><input class="pm-kv-val" placeholder="' +
        valPh +
        '" value="' +
        escHtml(val) +
        '"></td>' +
        '<td style="width:30px"><button class="pm-kv-del-btn"><i class="lni lni-close"></i></button></td>';
      tr.querySelector(".pm-kv-del-btn").addEventListener("click", function () {
        tr.remove();
      });
    ...
    function getRequestData() {
      ...
      if (authType === "bearer" && _authToken && _authToken.value.trim()) {
        ...
        headers["Authorization"] =
          "Basic " + btoa(_authUser.value + ":" + _authPass.value);
      }
  ...
  var ResponseViewer = (function () {
    ...
    function renderResult(result) {
      ...
      if (_statusEl) {
        _statusEl.innerHTML =
          '<span class="pm-status-badge pm-status-' +
          cls +
          '">' +
          code +
          " " +
          escHtml(result.status_text || "") +
          "</span>";
      }
      ...
      if (_bodyEl) {
        if (result.error) {
          ...
          if (result.body_truncated) {
            ...
            note.style.cssText =
              "color:var(--c360-warning);font-size:var(--c360-text-xs);margin-top:4px";
      ...
      if (_headersEl) {
        ...
        if (!keys.length) {
          _headersEl.innerHTML =
            '<p style="color:var(--c360-text-muted);font-size:var(--c360-text-sm)">No headers.</p>';
        } else {
          _headersEl.innerHTML =
            '<table class="pm-headers-table"><thead><tr><th>Header</th><th>Value</th></tr></thead><tbody>' +
            keys
              .map(function (k) {
                return (
                  "<tr><td>" +
    ...
    function showEmpty() {
      if (_statusEl)
        _statusEl.innerHTML =
          '<span class="pm-status-badge pm-status-0">—</span>';
      if (_timeEl) _timeEl.textContent = "";
      if (_sizeEl) _sizeEl.textContent = "";
      if (_bodyEl) {
        _bodyEl.className = "pm-code-viewer";
        _bodyEl.textContent = "Hit Send to receive a response.";
      }
    }
  ...
  var HistoryManager = (function () {
    ...
    function init(listEl, onSelectCb) {
      ...
      _onSelect = onSelectCb;
      try {
        _history = JSON.parse(sessionStorage.getItem("pm_history") || "[]");
      } catch (_) {}
      _render();
    }
    ...
    function add(entry) {
      ...
      try {
        sessionStorage.setItem("pm_history", JSON.stringify(_history));
      } catch (_) {}
      _render();
    }
    ...
    function _render() {
      ...
      if (!_history.length) {
        _listEl.innerHTML =
          '<div style="padding:8px 16px;font-size:var(--c360-text-xs);color:var(--c360-text-muted)">No history yet.</div>';
        return;
      }
      _listEl.innerHTML = _history
        .slice(0, 20)
        .map(function (h, i) {
          return (
            '<div class="pm-history-item" data-idx="' +
            i +
            '">' +
            '<span class="pm-method-pill pm-method-' +
            escHtml(h.method) +
            ...
            escHtml(h.method) +
            "</span>" +
            '<span class="pm-history-url">' +
            escHtml(h.url) +
            "</span>" +
            '<span class="pm-history-time">' +
            escHtml(h.status || "") +
            "</span>" +
            "</div>"
          );
  ...
  var TabManager = (function () {
    ...
    function openTab(item) {
      ...
      var existing = _tabs.findIndex(function (t) {
        return (
          t.name === item.name &&
          t.method === (item.request && item.request.method)
        );
      });
      if (existing !== -1) {
        _activate(existing);
      ...
      _tabBarEl.innerHTML = _tabs
        .map(function (t, i) {
          ...
          return (
            ...
            method +
            "</span>" +
            '<span class="pm-req-tab-name">' +
            escHtml(t.name) +
            "</span>" +
            '<button class="pm-req-tab-close" data-close="' +
            i +
            '">&times;</button>' +
            "</div>"
          );
  ...
  var PostmanApp = {
    init: function (opts) {
      ...
      CollectionTree.init(
        ...
        function (item) {
          PostmanApp._onRequestSelect(item);
        },
      );
      RequestEditor.init({
      ...
      TabManager.init(document.getElementById("pm-req-tabs"), function (item) {
        PostmanApp._onRequestSelect(item);
      });
      HistoryManager.init(
      ...
      if (searchEl) {
        searchEl.addEventListener("input", function () {
          CollectionTree.filterTree(searchEl.value);
        });
      }
      ...
      respTabLinks.forEach(function (link) {
        link.addEventListener("click", function () {
          respTabLinks.forEach(function (l) {
            l.classList.remove("active");
          });
          respTabPanes.forEach(function (p) {
      ...
      edTabLinks.forEach(function (link) {
        link.addEventListener("click", function () {
          edTabLinks.forEach(function (l) {
            l.classList.remove("active");
          });
          edTabPanes.forEach(function (p) {
            p.classList.remove("active");
          });
          link.classList.add("active");
          var target = document.getElementById(link.dataset.target);
      ...
      // Load data
      Promise.all([CollectionTree.loadList(), EnvManager.loadList()]).then(
        function () {
