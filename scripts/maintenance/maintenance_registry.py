...
from .era_config import era_dir
from .era_patch_enricher import enrich_era
from .era_minor_updater import update_era_minors
from .era_readme_fixer import fix_era_readme_links
...
def _dry_run_enrich(era_idx: int) -> None:
    ...
    n = len(list(root.glob(pat))) if root.is_dir() else 0
    console.print(
        f"[yellow]Dry-run[/yellow]: would run enrich on era {era_idx} "
        f"(~{n} files matching {pat!r} under {root.name})"
    )
...
def _dry_run_update_minors(era_idx: int) -> None:
    ...
    n = sum(1 for p in root.glob("*.md") if rgx.match(p.name)) if root.is_dir() else 0
    ...
    *,
    ...
    if action == "enrich":
        if dry_run:
            ...
            return 0
        return enrich_era(era_idx, docs_root=DOCS_ROOT, apply=not dry_run)

    if action == "fix-readme-links":
        if dry_run:
            _dry_run_fix(era_idx)
            return 0
        return fix_era_readme_links(era_idx, docs_root=DOCS_ROOT, apply=not dry_run)

    if action == "update-minors":
        if dry_run:
            _dry_run_update_minors(era_idx)
            return 0
        return update_era_minors(era_idx, docs_root=DOCS_ROOT, apply=not dry_run)
