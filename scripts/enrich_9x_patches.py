#!/usr/bin/env python3
"""
Enrich docs/9. Contact360 Ecosystem integrations and Platform productization/9.N.P — *.md
with Micro-gate + Service task slices from era `9.x` *-ecosystem-productization-task-pack.md` files
...
PACK_KEY_TO_FILE = {
    "appointment360": "appointment360-ecosystem-productization-task-pack.md",
    "connectra": "connectra-ecosystem-productization-task-pack.md",
    "contact-ai": "contact-ai-ecosystem-productization-task-pack.md",
    "emailapis": "emailapis-ecosystem-productization-task-pack.md",
    "emailcampaign": "emailcampaign-ecosystem-productization-task-pack.md",
    "jobs": "jobs-ecosystem-productization-task-pack.md",
    "logsapi": "logsapi-ecosystem-productization-task-pack.md",
    "mailvetter": "mailvetter-ecosystem-productization-task-pack.md",
    "s3storage": "s3storage-ecosystem-productization-task-pack.md",
    "salesnavigator": "salesnavigator-ecosystem-productization-task-pack.md",
}

MINOR_PRIMARY_PACKS: dict[str, list[str]] = {
    "9.0": ["contact-ai", "appointment360", "emailapis", "connectra"],
    "9.1": ["logsapi", "appointment360", "contact-ai", "jobs"],
    "9.2": ["salesnavigator", "connectra", "emailcampaign", "emailapis"],
    "9.3": ["appointment360", "connectra", "jobs", "emailapis"],
    "9.4": ["appointment360", "contact-ai", "salesnavigator", "emailapis"],
    "9.5": ["connectra", "emailapis", "appointment360", "s3storage"],
    "9.6": ["emailcampaign", "jobs", "connectra", "appointment360"],
    "9.7": ["salesnavigator", "mailvetter", "emailapis", "appointment360"],
    "9.8": ["logsapi", "appointment360", "emailcampaign", "contact-ai"],
    "9.9": list(PACK_KEY_TO_FILE.keys()),
    "9.10": list(PACK_KEY_TO_FILE.keys()),
}
...
_ERA_LINE_BAD = re.compile(
    r"- \*\*Era:\*\* \[9\.\d+ — Contact360 Ecosystem integrations and Platform productization\]\(\./README\.md\)"
)
_ERA_LINE_GOOD = (
    "- **Era:** `9.x` ecosystem integrations — hub [`versions.md`](../versions.md) · minors start at "
    "[`9.0 — Ecosystem Foundation`](9.0%20%E2%80%94%20Ecosystem%20Foundation.md)"
)
...
def micro_gate_block(minor: str, _patch_digit: int) -> str:
    fe = {
        "9.0": "Ecosystem foundation — partner taxonomy, connector inventory, integration matrices.",
        "9.1": "Partner identity — OAuth/scoped credentials, partner registry.",
        "9.2": "Connector contracts — SDK surfaces, lifecycle hooks, compatibility tests.",
        "9.3": "Entitlement fabric — quotas, plans, enforcement across services.",
        "9.4": "Tenant boundary — isolation, noisy-neighbor controls, residency notes.",
        "9.5": "Self-serve control plane — integrations UX, workspace, onboarding.",
        "9.6": "Webhook reliability — signing, replay, DLQ, partner delivery SLAs.",
        "9.7": "Operational transparency — health dashboards, audit exports, support tooling.",
        "9.8": "Commercial guardrails — usage caps, billing alignment, partner terms.",
        "9.9": "Productization RC — `integration-era-rc.md` / cross-service ecosystem gate.",
        "9.10": "Buffer minor — ecosystem overflow patches when chartered.",
    }.get(minor, "See minor ecosystem scope in roadmap / minor doc.")
    rows = [
        (
            "**Contract**",
            "Connector lifecycle, entitlement model — `docs/backend/apis/` + integration matrices updated?",
            "Document at patch closeout.",
        ),
        (
            "**Service**",
            "Multi-tenant enforcement, connector adapters, webhook delivery — parity + smoke documented?",
            "Document smoke paths.",
        ),
        (
            "**Surface**",
            "Integrations UI, marketplace/admin, self-serve flows — delta?",
            "Document UX delta or N/A.",
        ),
        (
            "**Frontend**",
            "`docs/frontend/` hooks, partner surfaces, extension/email integrations touched?",
            fe + " Document at closeout.",
        ),
        (
            "**Data**",
            "Tenant lineage, `connector_id`, entitlement tables — `docs/backend/database/`?",
            "Document lineage or N/A.",
        ),
        (
            "**Ops**",
            "SLA runbooks, partner onboarding, `connectors-commercial.md` / integration RC evidence — delta?",
...
def main() -> None:
    if not ERA.is_dir():
        print("9.x era folder not found")
        return
    ...
    for path in sorted(ERA.glob("9.*.* — *.md")):
        enrich_one(path)
    print(f"Processed 9.x patch files in {ERA}")
