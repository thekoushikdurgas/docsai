"""
Local contact data ingestion module.

This module handles ingesting contact data from local CSV files.
"""
...
class LocalContactIngester(LocalFileIngester):
    """Ingester for contact data from local CSV files."""
        ...
        for data in data_list:
            try:
                ...
                contact_metadata = {
                    ...
                    "linkedin_sales_url": data.get('person_linkedin_sales_url') or "_",
...
def ingest_contacts_from_local(
    path: str,
    batch_size: int = DEFAULT_BATCH_SIZE,
    max_threads: int = DEFAULT_MAX_THREADS
) -> None:
    """
    Ingest contact data from a local CSV file.
    ...
    """
    ingester = LocalContactIngester(path, batch_size=batch_size, max_threads=max_threads)
    with ingester:
        ingester.ingest()
...
if __name__ == "__main__":
    ...
    ingest_contacts_from_local('apollo_sample_5000_cleaned.csv')
