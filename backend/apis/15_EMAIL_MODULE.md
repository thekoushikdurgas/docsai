...
The **email finder export** (CSV → find emails → CSV) and **email verify export** (CSV → verify emails → CSV) operations are long-running jobs handled by the **Tkdjob** scheduler. Use the [Jobs Module](16_JOBS_MODULE.md) instead of the Email module:
...
After creating a job, use `jobs.job(jobId)` to poll status or query the tkdjob API directly. The Jobs module stores a local record in `scheduler_jobs` for ownership and listing.
