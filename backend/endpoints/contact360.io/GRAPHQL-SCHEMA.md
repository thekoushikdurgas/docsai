...
- **`benchCandidates`:** `BenchCandidateQuery` / `BenchCandidateMutation` — user-scoped bench list (`bench_candidates` Postgres); auto-enrichment via job.server Apify pipeline; `benchCandidateMatchedJobs` maps enriched profile → `list_jobs` filters.
