...
- `jobs.api.md` — `contact360.io/jobs` / tkdjob
...
- `emailcampaign.api.md` (REST campaign service index; deep contract still in `apis/` + matrices)

## Scope

- Documents service-local routes, auth models, payload contracts, and ops notes.
- Complements (not replaces) `docs/backend/apis/` module docs and endpoint matrices.
...
- Registry: [docs/backend/README.md](../README.md#codebase-analysis-registry) — each row lists `services.apis/*.md` where it applies.
...
- Deep route-level detail may still live in `docs/backend/apis/`, `docs/codebases/*-codebase-analysis.md`, and Postman folders—keep this directory as the **service hub** and navigation layer.
