...
```mermaid
flowchart LR
    patchNode["1.3.5 — Review"] --> workNode["Admin list"]
...
## Tasks
### Contract
- ✅ Completed: Admin can list payment submissions for review:
- ✅ Completed: Admin query surface ties to `AdminQuery.paymentSubmissions` / `AdminQuery.users` patterns.

### Service
- ✅ Completed: Ensure review list returns submissions with fields needed for decision:
- ✅ Completed: proof_url, amount, status, reviewed_by (when decided).

### Surface
- ✅ Completed: Admin UI shows payment queue with approve/decline controls and reason inputs.

### Data
- ✅ Completed: Confirm filterability by status (`pending/approved/declined`) and sorting stability.

### Ops
- ✅ Completed: UI + authorization test:
- ✅ Completed: admin sees only eligible items, and approves update status deterministically.
