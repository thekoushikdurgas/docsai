...
```mermaid
flowchart LR
    patchNode["1.5.1 — Wave"] --> workNode["Threshold rules"]
...
## Tasks
### Contract
- ✅ Completed: Define threshold bands:
- ✅ Completed: remaining > 50% (green), 20–50% (amber), < 20% (red), consistent with usage/credit progress display.

### Service
- ✅ Completed: Compute threshold state from server-derived remaining/limit values (no stale client-only math).

### Surface
- ✅ Completed: Update `CreditBudgetAlerts` display state for each band.

### Data
- ✅ Completed: Keep the threshold function stable across refresh so UI isn’t jittery.

### Ops
- ✅ Completed: Test threshold boundary values (exact 50%, 20%, 0).
