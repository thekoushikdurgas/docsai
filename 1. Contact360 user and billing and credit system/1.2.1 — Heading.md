...
```mermaid
flowchart LR
    patchNode["1.2.1 — Heading"] --> workNode["Usage query MVP"]
...
## Tasks
### Contract
- ✅ Completed: Lock `UsageQuery.usage(feature)` return schema: `UsageResponse.features[].{used,limit,remaining,resetAt}` per [`docs/backend/apis/09_USAGE_MODULE.md`](../backend/apis/09_USAGE_MODULE.md).

### Service
- ✅ Completed: Ensure `usage` implementation derives from the same credits ledger columns (`credits.total`, `credits.consumed`, `credits.reset_at`).

### Surface
- ✅ Completed: Usage page binds `graphql/GetUsage` and renders cards/table values via `useUsagePage`.

### Data
- ✅ Completed: Usage rows/limits exist for first tracked features (auto-create behavior matches usage module expectations).

### Ops
- ✅ Completed: Load smoke: usage query under typical dashboard access pattern (no N+1).
