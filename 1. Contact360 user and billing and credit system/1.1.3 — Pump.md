...
```mermaid
flowchart LR
    patchNode["1.1.3 — Pump"] --> workNode["Job enqueue"]
...
## Tasks
### Contract
- ✅ Completed: Job enqueue uses `JobMutation.createEmailFinderExport` / `createEmailVerifyExport` with:
- ✅ Completed: `job_node.data.user_uuid`,
- ✅ Completed: billing correlation identifiers for later credit deduction.

### Service
- ✅ Completed: jobs scheduler enqueues `open` jobs and sets DAG edges in jobs DB.
- ✅ Completed: Worker pool transitions jobs and records `job_events` with billing trace data.

### Surface
- ✅ Completed: UI shows enqueue success and transitions to “processing” state with progress.

### Data
- ✅ Completed: `job_node` and `job_events` store `user_uuid` and `correlation_id`; row counts are persisted per checkpoint updates.

### Ops
- ✅ Completed: Trace test: capture `job_uuid` → `job_events` → reconciliation inputs.
