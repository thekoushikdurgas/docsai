...
```mermaid
flowchart LR
    patchNode["1.4.0 — Gauge"] --> workNode["Charter"]
...
## Tasks
### Contract
- ✅ Completed: Define `UsageQuery.usage(feature)` and the expected `UsageResponse.features[]` shape for the usage ledger UI:
- ✅ Completed: `used`, `limit`, `remaining`, `resetAt` per [`docs/backend/apis/09_USAGE_MODULE.md`](../backend/apis/09_USAGE_MODULE.md).

### Service
- ✅ Completed: Implement usage ledger computation by joining credits + plan limits:
- ✅ Completed: `credits (total/consumed/reset_at)` reconciles with `plans.limits` (JSON feature → limit).

### Surface
- ✅ Completed: Usage page renders a feature breakdown and remaining/limit bar (“first-class visibility into spend”):
- ✅ Completed: components: `UsageOverview`, `UsageLimitsTable`.

### Data
- ✅ Completed: Validate lineage columns:
- ✅ Completed: `credits.total`, `credits.consumed`, `credits.reset_at`,
- ✅ Completed: `plans.limits` (JSONB).

### Ops
- ✅ Completed: Spot-check ledger math for one test user by comparing finder/verifier actions vs usage results.
