...
## Tasks
### Contract
- ✅ Completed: `UsageQuery usage(feature)` returns `UsageResponse.features[]` with `used`, `limit`, `remaining`, `resetAt` as documented in [`docs/backend/apis/09_USAGE_MODULE.md`](../backend/apis/09_USAGE_MODULE.md).
- ✅ Completed: Feature-name normalization behavior is consistent with deduction feature keys.

### Service
- ✅ Completed: Usage aggregation is performant (avoid N+1):
- ✅ Completed: ensure usage query reads from credits/ledger in a single path.
- ✅ Completed: `remaining` math matches credits (`total` minus `consumed`, and `999999/unlimited` conventions if applicable).

### Surface
- ✅ Completed: `UsageOverview` page binds `graphql/GetUsage` and renders:
- ✅ Completed: remaining/limit cards,
- ✅ Completed: reset notice (resetAt).
- ✅ Completed: Credits badge/header refreshes after finder/verifier.

### Data
- ✅ Completed: credits ledger consistency:
- ✅ Completed: `credits.total`, `credits.consumed`, and `credits.reset_at` are the sources of truth.

### Ops
- ✅ Completed: Reconciliation spot-check:
- ✅ Completed: run N finder/verifier actions and compare ledger deltas to `usage(feature)` output.
