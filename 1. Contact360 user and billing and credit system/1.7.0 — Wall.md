...
```mermaid
flowchart LR
    patchNode["1.7.0 — Wall"] --> workNode["Charter"]
...
## Tasks
### Contract
- ✅ Completed: Define throttling contract for public GraphQL:
- ✅ Completed: `GraphQLRateLimitMiddleware` governs `/graphql`,
- ✅ Completed: define config key `GRAPHQL_RATE_LIMIT_REQUESTS_PER_MINUTE` semantics.

### Service
- ✅ Completed: Confirm middleware composes correctly with:
- ✅ Completed: `GraphQLMutationAbuseGuardMiddleware`,
- ✅ Completed: `GraphQLIdempotencyMiddleware`.

### Surface
- ✅ Completed: App provides error toast + user-friendly “try again” guidance for rate limits.

### Data
- ✅ Completed: Optional: define whether block events are persisted (later in logs.api) or kept transient.

### Ops
- ✅ Completed: Baseline test that 429 triggers before DB exhaustion (no cascading failures).
