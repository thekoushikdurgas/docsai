...
```mermaid
flowchart LR
    patchNode["1.3.1 — Lock"] --> workNode["Authorization"]
...
## Tasks
### Contract
- ✅ Completed: Enforce admin-only authorization for approval/decline:
- ✅ Completed: `approvePayment` / `declinePayment` require `require_admin` guards (and audit wrapping).

### Service
- ✅ Completed: Reject non-admin calls to approve/decline with structured GraphQL auth errors.
- ✅ Completed: Ensure user cannot approve their own submissions; ensure admin ID is recorded (`reviewed_by`).

### Surface
- ✅ Completed: UI: admin-side approval/decline path is role-gated (no action buttons for non-admin).

### Data
- ✅ Completed: Ensure `payment_submissions.reviewed_by` is set only on approve/decline.

### Ops
- 📌 Planned: **[appointment360]** — refine duplicate task (was: ✅ completed: pen-test gate:) | patch `1.3.1` band `1` | reason: specialize this file vs sibling patches; see docs/codebases/appointment360-codebase-analysis.md
- ✅ Completed: “non-admin cannot call admin mutations” scenario is validated.
