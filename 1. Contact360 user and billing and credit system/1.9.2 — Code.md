...
```mermaid
flowchart LR
    patchNode["1.9.2 — Code"] --> workNode["TOTP"]
...
## Tasks
### Contract
- ✅ Completed: Define the OTP verification variable contract:
- ✅ Completed: `verify2FA(code: String!)` expects correct code format.

### Service
- ✅ Completed: Implement constant-time verify behavior (avoid oracle timing leaks).
- ✅ Completed: Confirm placeholder TOTP generation caveats are documented in implementation notes (module doc indicates placeholder; ensure it won’t leak in production config).

### Surface
- ✅ Completed: OTP input UI accepts digits with correct validation and clear “try again” on mismatch.

### Data
- ✅ Completed: No new schema required; verification updates the `verified` flag.

### Ops
- 📌 Planned: **[appointment360]** — refine duplicate task (was: ✅ completed: negative tests:) | patch `1.9.2` band `2` | reason: specialize this file vs sibling patches; see docs/codebases/appointment360-codebase-analysis.md
- ✅ Completed: wrong TOTP must not enable 2FA and must be logged safely.
