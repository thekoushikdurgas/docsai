...
## Tasks
### Contract
- ✅ Completed: Define a discrepancy signal:
- ✅ Completed: when client-side cached usage differs from a fresh server `usage(feature)` read.

### Service
- ✅ Completed: Provide a mechanism for the UI to detect mismatch:
- ✅ Completed: either via explicit response metadata or via strict re-fetch gating after state changes.

### Surface
- ✅ Completed: If discrepancy is detected, show an alert state instead of stale “remaining” numbers.

### Data
- ✅ Completed: Ensure discrepancy detection compares the same feature key normalization.

### Ops
- ✅ Completed: Reconciliation test:
- ✅ Completed: force a stale client state and confirm the next refresh corrects it safely.
