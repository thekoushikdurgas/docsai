...
- **Status:** planned (slice; see also **`1.1`** for shipped bulk+billing track)  
...
- 📌 Planned: Billing types in `docs/backend/apis/10_BILLING_MODULE.md` (or equivalent).  
- 📌 Planned: Idempotency key header for subscribe/purchase/proof.

### Service

- 📌 Planned: Server-side validation of proof metadata; no double-credit on retry.  
- 📌 Planned: Decline path: reason code + user-visible state.

### Surface

- 📌 Planned: `UpiPaymentModal` / billing service hooks; admin payment list.

### Data

- 📌 Planned: `payment_submissions` (or equivalent) + linkage to `users.uuid`.  
- 📌 Planned: logs.api audit events: payment_received, payment_approved.

### Ops

- 📌 Planned: Manual reconciliation runbook for stuck submissions.
