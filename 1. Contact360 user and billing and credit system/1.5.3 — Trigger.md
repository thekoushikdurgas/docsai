...
```mermaid
flowchart LR
    patchNode["1.5.3 — Trigger"] --> workNode["Low credit"]
...
## Tasks
### Contract
- ✅ Completed: Define trigger condition:
- ✅ Completed: when usage/remaining crosses into low-credit band.

### Service
- ✅ Completed: Ensure trigger uses `usage(feature)`/credits remaining (authoritative).

### Surface
- ✅ Completed: Trigger causes:
- ✅ Completed: `CreditBudgetAlerts` visible state,
- ✅ Completed: payment CTA deep-linking to `/billing`.

### Data
- ✅ Completed: Ensure feature-key alignment between UI triggers and backend credit buckets.

### Ops
- ✅ Completed: Edge test: credits reach threshold then drop again; cue triggers again only when warranted.
