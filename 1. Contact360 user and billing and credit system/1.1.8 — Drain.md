...
## Tasks
### Contract
- ✅ Completed: Gateway reconciliation contract ties job completion totals to usage ledger:
- ✅ Completed: charged-row totals must reconcile with `usage(feature)` within tolerances.

### Service
- ✅ Completed: Reconcile job `job_uuid` results → update credits/usage and optionally emit logs.api billing events.

### Surface
- ✅ Completed: Provide a visible reconciliation “badge”/hint (or at least accurate post-job credits remaining) so user sees consistency.

### Data
- ✅ Completed: Ensure correlation IDs exist end-to-end so logs and ledgers are queryable.

### Ops
- ✅ Completed: Divergence checks:
- ✅ Completed: charged_rows sum vs gateway usage over a sliding window,
- ✅ Completed: alert/monitor when variance crosses threshold.
