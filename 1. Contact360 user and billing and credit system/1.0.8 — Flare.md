...
## Tasks
### Contract
- ✅ Completed: Ensure JWT session is usable by extension-style clients:
- ✅ Completed: Authorization header / token refresh flow remains consistent.
- ✅ Completed: Clarify whether extension client uses the same `refreshToken` contract as the app.

### Service
- ✅ Completed: JWT context extraction is robust regardless of client (browser app vs extension).
- ✅ Completed: Refresh-to-access flow succeeds after token expiry mid-session.

### Surface
- ✅ Completed: `AuthContext` behavior (token refresh + session state updates) supports extension call timing:
- ✅ Completed: no “stuck logged out” state after refresh.

### Data
- ✅ Completed: Token blacklist checks prevent revoked access from being accepted after refresh cycles.

### Ops
- ✅ Completed: Simulate token expiry mid-flow and confirm:
- ✅ Completed: refresh recovers session,
- ✅ Completed: revoked tokens do not recover session.
