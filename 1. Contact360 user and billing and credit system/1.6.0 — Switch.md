...
## Tasks
### Contract
- ✅ Completed: Admin control plane contract is defined:
- ✅ Completed: admin query surfaces (`AdminQuery.users`, `AdminQuery.userStats`, etc.),
- ✅ Completed: admin mutation surfaces (`AdminMutation.creditUser`, `AdminMutation.adjustCredits`, plus payment approve/decline paths).

### Service
- ✅ Completed: Enforce admin-only access checks for all admin operations (RBAC guard correctness is MVP).

### Surface
- ✅ Completed: Admin UI shell loads:
- ✅ Completed: admin console navigation,
- ✅ Completed: base templates and payment/user management surfaces.

### Data
- ✅ Completed: Ensure audit fields exist for any adjustment:
- ✅ Completed: actor id and reason are persisted for downstream evidence.

### Ops
- ✅ Completed: Smoke: admin can view a user list and perform a no-op adjustment review path without breaking RBAC.
