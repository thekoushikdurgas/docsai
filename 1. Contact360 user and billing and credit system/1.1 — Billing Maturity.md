...
- 📌 Planned: Credit-aware **job create** payload; pre-check credits before enqueue.  
- 📌 Planned: **Billing** mutations: `subscribe`, `purchaseAddon`, `submitPaymentProof` — idempotency keys per middleware.  
- 📌 Planned: Bulk **row-level** failure vs credit charge policy documented.

### Service

- 📌 Planned: Jobs: attach **user_uuid** / billing metadata on `job_node.data`.  
- 📌 Planned: Gateway: reconcile **job completion** with usage ledger.  
- 📌 Planned: emailapis: bulk **status vocabulary** aligned with billing reconciliation.

### Surface

- 📌 Planned: Bulk upload UI, job progress, credit warning before start.  
- 📌 Planned: Billing / UPI or proof modal wiring per roadmap 1.3 code pointers.

### Data

- 📌 Planned: `job_events` include correlation IDs for billing.  
- 📌 Planned: Payment submission rows if applicable.

### Ops

- 📌 Planned: Alert on job failure spike affecting paid users.  
- 📌 Planned: Runbook: retry vs refund credit policy.
