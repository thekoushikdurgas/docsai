...
## Tasks
### Contract
- ✅ Completed: Confirm plan limit enforcement semantics:
- ✅ Completed: free/pro tier limits map from `plans.limits` JSONB,
- ✅ Completed: unlimited convention is consistent (`limit=999999` → `remaining=-1`).

### Service
- ✅ Completed: Ensure limit mapping from `plans` is applied per feature key used by deduction.

### Surface
- ✅ Completed: Remaining credits / limit bar uses server-derived `remaining` and displays quota thresholds clearly.

### Data
- ✅ Completed: `plans` seed data and `credits.feature` naming stay aligned; mismatch is treated as a contract failure.

### Ops
- ✅ Completed: Test matrix:
- ✅ Completed: FreeUser vs ProUser shows correct limit/remaining outputs.
