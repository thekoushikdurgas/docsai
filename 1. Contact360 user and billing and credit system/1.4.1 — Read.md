...
```mermaid
flowchart LR
    patchNode["1.4.1 — Read"] --> workNode["Query MVP"]
...
## Tasks
### Contract
- ✅ Completed: Confirm `UsageQuery.usage(feature)` returns correct values for both:
- ✅ Completed: all features list,
- ✅ Completed: single feature filter.

### Service
- ✅ Completed: Ensure query implementation avoids N+1 by using DataLoaders or bulk reads (Usage page scale sanity).

### Surface
- ✅ Completed: `usage/page.tsx` binds `graphql/GetUsage` and displays values consistently in cards and/or table view.

### Data
- ✅ Completed: Usage response is sourced directly from credits ledger columns (no drift from cached UI state).

### Ops
- ✅ Completed: Load smoke: usage query completes within expected latency for MVP dashboard traffic.
