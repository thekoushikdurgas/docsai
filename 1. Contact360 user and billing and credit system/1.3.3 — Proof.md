...
## Tasks
### Contract
- ✅ Completed: Freeze s3storage “proof object class policy” for payment screenshots:
- ✅ Completed: required content-type and max size,
- ✅ Completed: deterministic prefix and metadata.json linkage.
- ✅ Completed: File upload contract is compatible with billing proof UI:
- ✅ Completed: Billing UI sends the file + `upi-reference` text to gateway.

### Service
- ✅ Completed: s3storage upload endpoints validate per object class and store proof under prefix model (e.g., `{bucket_id}/upload/`).
- ✅ Completed: Metadata worker updates `metadata.json` to support later verification/replay.

### Surface
- ✅ Completed: `PaymentModal` validates screenshot input and provides clear error states for unsupported formats.

### Data
- ✅ Completed: Persist `payment_submissions.proof_url` only after successful s3storage upload.

### Ops
- ✅ Completed: Upload negative tests:
- ✅ Completed: invalid file types rejected,
- ✅ Completed: oversized proofs rejected with a safe error envelope.
