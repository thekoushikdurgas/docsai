...
```mermaid
flowchart LR
    patchNode["1.9.0 — Factor"] --> workNode["Charter"]
...
## Tasks
### Contract
- ✅ Completed: Freeze 2FA contract surface:
- ✅ Completed: `TwoFactorQuery.get2FAStatus`,
- ✅ Completed: `TwoFactorMutation.setup2FA`, `verify2FA`, `disable2FA`,
- ✅ Completed: backup codes flow: `regenerateBackupCodes`.
  (per [`docs/backend/apis/27_TWO_FACTOR_MODULE.md`](../backend/apis/27_TWO_FACTOR_MODULE.md).)

### Service
- ✅ Completed: Implement TOTP setup with secure secret hashing and record creation/update behavior for the authenticated user only.

### Surface
- ✅ Completed: Profile/security UX includes 2FA status + enable/disable flows:
- ✅ Completed: endpoints bind to `twoFactorService` via `use2FA`,
- ✅ Completed: UI includes `TwoFactorModal` (QR + OTP).

### Data
- ✅ Completed: Validate 2FA data lineage for the 2FA record:
- ✅ Completed: secret hash storage,
- ✅ Completed: enabled/verified flags,
- ✅ Completed: backup codes hash.

### Ops
- ✅ Completed: Enrollment happy-path smoke test:
- ✅ Completed: Setup2FA → Verify2FA → enabled+verified state reflected in profile.
