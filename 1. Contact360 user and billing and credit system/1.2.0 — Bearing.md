...
```mermaid
flowchart LR
    patchNode["1.2.0 — Bearing"] --> workNode["Release charter"]
...
## Tasks
### Contract
- ✅ Completed: Document GraphQL operation surface for the minor:
- ✅ Completed: usage: `UsageQuery.usage(feature)` per [`docs/backend/apis/09_USAGE_MODULE.md`](../backend/apis/09_USAGE_MODULE.md),
- ✅ Completed: admin stats: `AdminQuery.userStats` / `AdminQuery.users` / `AdminQuery.logStatistics` per [`docs/backend/apis/13_ADMIN_MODULE.md`](../backend/apis/13_ADMIN_MODULE.md),
- ✅ Completed: optional notification listing: `NotificationsQuery.notifications` per [`docs/backend/apis/05_NOTIFICATIONS_MODULE.md`](../backend/apis/05_NOTIFICATIONS_MODULE.md).
- ✅ Completed: Define throttling/rate-limit contract readiness (429 error envelope) aligned with `GraphQLRateLimitMiddleware` config.

### Service
- ✅ Completed: Implement MVP usage aggregation path and ensure it is the same underlying source as credit deduction (`credits` → `usage(feature)`).
- ✅ Completed: Ensure admin stats and notification listing are RBAC-safe (role-gated resolvers).

### Surface
- ✅ Completed: Confirm UI entry points exist:
- ✅ Completed: usage dashboard components,
- ✅ Completed: `CreditBudgetAlerts` placement in app layout (driven by remaining credit computed from usage).

### Data
- ✅ Completed: Ensure logs/audit evidence for admin and billing/auth events are queryable from logs.api canonical store (S3 CSV lineage).

### Ops
- ✅ Completed: Combined DoD charter: “usage + low-credit cues + admin views load” before landing child minors (`1.4`–`1.7`).
