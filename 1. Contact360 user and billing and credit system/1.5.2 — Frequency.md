...
```mermaid
flowchart LR
    patchNode["1.5.2 — Frequency"] --> workNode["Debounce"]
...
## Tasks
### Contract
- ✅ Completed: Debounce/dedupe policy:
- ✅ Completed: “show low-credit alert once per session” (or once per credit spend action) for MVP.

### Service
- ✅ Completed: Ensure alert generation isn’t triggered repeatedly during intermediate refreshes.

### Surface
- ✅ Completed: UI does not spam: alert components and toasts are rate-limited client-side (or gated by response changes).

### Data
- ✅ Completed: Track suppression scope (session-scoped key) if no persistence is used.

### Ops
- ✅ Completed: Test: repeated polling/refetch does not create duplicate alerts.
