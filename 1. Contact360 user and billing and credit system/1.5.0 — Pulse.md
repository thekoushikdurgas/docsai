...
```mermaid
flowchart LR
    patchNode["1.5.0 — Pulse"] --> workNode["Charter"]
...
## Tasks
### Contract
- ✅ Completed: Define “low-credit” cue semantics and their UI entry points:
- ✅ Completed: threshold derived from remaining credits (prefer `usage(feature)` / server-derived remaining).
- ✅ Completed: Define “payment success” cue semantics after admin approval:
- ✅ Completed: gateway state refreshed so credits reflect ledger reality.

### Service
- ✅ Completed: Ensure balance refresh after billing approve so the success toast is consistent with credit updates.

### Surface
- ✅ Completed: Place `CreditBudgetAlerts` in the app layout (via `MainLayout`) and wire success feedback to the billing flow.
- ✅ Completed: Billing UI success cue uses `PaymentModal` flow (“Admin approves → credits added automatically”).

### Data
- ✅ Completed: MVP can keep notification display state ephemeral (session-scoped); optional: later wire `NotificationPreferences`.

### Ops
- ✅ Completed: KPI note: correctness of alerting (false positives/false negatives) before moving to `1.6`.
