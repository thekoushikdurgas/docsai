...
- ✅ Completed: ✅ Completed: 📌 Planned: `usage`, `analytics`, `admin` GraphQL operations documented.
- ✅ Completed: ✅ Completed: 📌 Planned: Rate limit: config keys + error shape for throttled clients.

- ✅ Completed: 📌 Planned: **[appointment360]** — refine duplicate task (was: 📌 planned: **[architecture]** — product **graphql** remains …) | patch `1.2.0` band `0` | reason: specialize this file vs sibling patches; see docs/codebases/appointment360-codebase-analysis.md
### Service

- ✅ Completed: ✅ Completed: 📌 Planned: Usage **aggregation** or query path performant for MVP scale.
- ✅ Completed: ✅ Completed: 📌 Planned: Admin mutations: **creditUser**, **adjustCredits** — audit-wrapped.

- ✅ Completed: 📌 Planned: **[appointment360]** — refine duplicate task (was: 📌 planned: **[architecture]** — **go/gin satellites** in sco…) | patch `1.2.0` band `0` | reason: specialize this file vs sibling patches; see docs/codebases/appointment360-codebase-analysis.md
### Surface

- ✅ Completed: ✅ Completed: 📌 Planned: [`docs/governance.md`](../governance.md) pointers: `usage/page.tsx`, `CreditBudgetAlerts`, admin clients.

- ✅ Completed: 📌 Planned: **[appointment360]** — refine duplicate task (was: 📌 planned: **[architecture]** — **next.js** customer surface…) | patch `1.2.0` band `0` | reason: specialize this file vs sibling patches; see docs/codebases/appointment360-codebase-analysis.md
### Data

- ✅ Completed: ✅ Completed: 📌 Planned: logs.api **S3 CSV** events for billing/auth if admin views query logs.

- ✅ Completed: 📌 Planned: **[appointment360]** — refine duplicate task (was: 📌 planned: **[architecture]** — **postgresql-first** per `do…) | patch `1.2.0` band `0` | reason: specialize this file vs sibling patches; see docs/codebases/appointment360-codebase-analysis.md
### Ops

- ✅ Completed: ✅ Completed: 📌 Planned: Dashboard health for rate-limit metrics optional.

- ✅ Completed: 📌 Planned: **[appointment360]** — refine duplicate task (was: 📌 planned: **[architecture]** — **observability**: correlate…) | patch `1.2.0` band `0` | reason: specialize this file vs sibling patches; see docs/codebases/appointment360-codebase-analysis.md
## Task Breakdown
...
## Master-plan execution — `1.2.0` five-track closure

Structured delivery checklist (maps to Contract / Service / Surface / Data / Ops). **Docs paths** use `graphql.modules/` (not legacy `apis/`).

| Track | Deliverable | Evidence / pointers |
| --- | --- | --- |
| **Contract** | Usage + analytics + admin read models documented and aligned with era matrices | [`09_USAGE_MODULE.md`](../backend/graphql.modules/09_USAGE_MODULE.md), [`18_ANALYTICS_MODULE.md`](../backend/graphql.modules/18_ANALYTICS_MODULE.md), [`14_BILLING_MODULE.md`](../backend/graphql.modules/14_BILLING_MODULE.md), [`13_ADMIN_MODULE.md`](../backend/graphql.modules/13_ADMIN_MODULE.md); [`endpoints/appointment360_endpoint_era_matrix.md`](../backend/endpoints/appointment360_endpoint_era_matrix.md) |
| **Service** | CORS hardening + predictable GraphQL throttle behavior | FastAPI `CORSMiddleware` + ordering in `contact360.io/api/app/main.py`; nginx pass-through `contact360.io/api/deploy/nginx-appointment360.conf`; ops notes in `contact360.io/api/docs/deploy/CORS_FIX_DEPLOYMENT.md`. Rate limits: `GraphQLRateLimitMiddleware` (see gateway codebase). **2FA quality:** resolver + `two_factor` table — [`graphql.modules`](../backend/graphql.modules/README.md) + [`docs/backend/database/two_factor.sql`](../backend/database/two_factor.sql) |
| **Surface** | Usage summary, low-credit warning, payment-success UX | Dashboard billing/usage under `contact360.io/app/src/components/billing/` (e.g. usage panels, `CreditBudgetAlerts`, post-checkout messaging). Wire to `usage` / `featureOverview` / billing mutations per modules above. |
| **Data** | Credits, usage, aggregates, audit trail | [`feature_usage.sql`](../backend/database/feature_usage.sql), [`user_profiles.sql`](../backend/database/user_profiles.sql), [`performance_metrics.sql`](../backend/database/performance_metrics.sql); optional [`crud/`](../backend/database/crud/README.md) examples. Extend lineage markdown in-repo when adding columns. |
| **Ops** | Feature flags + monitoring for new queries | Env: `ALLOWED_ORIGINS`, `GRAPHQL_RATE_LIMIT_*` (exact keys per gateway `config`). Dashboard/alerting for throttle 429s and usage query latency. |
...
| --- | --- |
| **Contract** | Did any GraphQL / REST contract change? Diff vs `docs/backend/graphql.modules/`; billing idempotency keys documented? |
...
- Document GraphQL operation surface for the minor:
  - usage: `UsageQuery.usage(feature)` per [`docs/backend/graphql.modules/09_USAGE_MODULE.md`](../backend/apis/09_USAGE_MODULE.md),
  - admin stats: `AdminQuery.userStats` / `AdminQuery.users` / `AdminQuery.logStatistics` per [`docs/backend/graphql.modules/13_ADMIN_MODULE.md`](../backend/apis/13_ADMIN_MODULE.md),
  - optional notification listing: `NotificationsQuery.notifications` per [`docs/backend/graphql.modules/05_NOTIFICATIONS_MODULE.md`](../backend/apis/05_NOTIFICATIONS_MODULE.md).
...
- Lock `UsageQuery.usage(feature)` return schema: `UsageResponse.features[].{used,limit,remaining,resetAt}` per [`docs/backend/graphql.modules/09_USAGE_MODULE.md`](../backend/apis/09_USAGE_MODULE.md).
...
- Confirm analytics chart data endpoints are defined for dashboard visualization:
  - `AnalyticsQuery.performanceMetrics` / `aggregateMetrics` per [`docs/backend/graphql.modules/18_ANALYTICS_MODULE.md`](../backend/apis/18_ANALYTICS_MODULE.md),
...
- Notifications wiring uses `NotificationsQuery.notifications` and notification mutation endpoints:
  ...
  (All per [`docs/backend/graphql.modules/05_NOTIFICATIONS_MODULE.md`](../backend/apis/05_NOTIFICATIONS_MODULE.md).)
...
- Admin stats queries align with [`docs/backend/graphql.modules/13_ADMIN_MODULE.md`](../backend/apis/13_ADMIN_MODULE.md):
