...
```mermaid
flowchart LR
    patchNode["1.2.4 — Course"] --> workNode["Low-credit UX"]
...
## Tasks
### Contract
- ✅ Completed: Define the “low credit” signal as derived from usage remaining:
- ✅ Completed: source of truth: `usage(feature)` remaining (not stale client state).

### Service
- ✅ Completed: Ensure the server emits/derives low-credit status consistently after billing approve or credit spend.

### Surface
- ✅ Completed: `CreditBudgetAlerts` displays threshold cues and routes user to billing when appropriate.

### Data
- ✅ Completed: Prevent duplicate cues (policy can be session-scoped for MVP).

### Ops
- ✅ Completed: Monitor false-positive rate after threshold tuning (KPI: correct alerting on state changes).
