...
```mermaid
flowchart LR
    patchNode["1.1.2 — Valve"] --> workNode["Credit pre-check"]
...
## Tasks
### Contract
- ✅ Completed: Job create contract must require credit pre-check behavior (fail fast) so job processors never run without a ledger plan.

### Service
- ✅ Completed: Gateway checks `credits` / `usage(feature)` before creating a DAG; rejects with structured error if insufficient.
- ✅ Completed: Ensure feature enum naming matches deduction keys used by Email jobs.

### Surface
- ✅ Completed: Before enqueue: show “not enough credits” state and route user to billing if applicable.

### Data
- ✅ Completed: Usage/credits math is consistent with `credits.total - credits.consumed` and any unlimited convention.

### Ops
- ✅ Completed: Automated negative test: insufficient credits → no `job_node` row and no downstream emailapis calls.
