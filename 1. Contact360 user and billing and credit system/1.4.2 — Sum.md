...
## Tasks
### Contract
- ✅ Completed: Define how totals are aggregated across features (for “sum” cards/totals).

### Service
- ✅ Completed: Ensure aggregation logic doesn’t introduce rounding errors:
- ✅ Completed: totals derived from `credits.consumed/total` rather than client estimates.

### Surface
- ✅ Completed: Aggregate UI elements update from the same usage read path (no separate endpoints).

### Data
- ✅ Completed: Validate that monthly reset conventions reflected in `resetAt` are applied consistently to totals.

### Ops
- ✅ Completed: Regression: run multiple feature actions and confirm sums match ledger deltas.
