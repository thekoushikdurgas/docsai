...
- 📌 Planned: **contact360.io/api** — `app/graphql/modules/linkedin/` does not implement a `linkedInImportCSV` or bulk LinkedIn upload mutation — add a `linkedInBulkImport`-style mutation when product scope is confirmed (legacy standalone job scheduler inputs removed).
