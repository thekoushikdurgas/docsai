...
| Logs API | `lambda/logs.api/` | active | Centralized logs/audit (S3 CSV with cache) |
| S3 Storage | `lambda/s3storage/` | active | Uploads, exports, artifacts |
| Sales Navigator | `backend(dev)/salesnavigator/` (or `lambda/salesnavigator/`) | active | SN ingestion workflows |
| Contact AI | `backend(dev)/contact.ai/` (or `lambda/contact.ai/`) | active | AI workflows (e.g. Gemini) |
| Resume AI | `backend(dev)/resumeai/` | active | Resume-related AI workflows (when checkout present) |
| TKD Job | `contact360.io/jobs/` | active | Async jobs, Kafka, DAGs; older docs reference `lambda/tkdjob/` |
...
| Logs API | `lambda/logs.api/` | Python | Operational and audit log ingestion |
| S3 Storage | `lambda/s3storage/` | Python | File artifact lifecycle on S3 |
| Sales Navigator | `backend(dev)/salesnavigator/` | Python | Sales Navigator ingestion workflows |
| Contact AI | `backend(dev)/contact.ai/` | Python | AI pipeline logic and model integration |
| Resume AI | `backend(dev)/resumeai/` | Python | Resume AI features (when present) |
| TKD Job | `contact360.io/jobs/` | Python | Job orchestration, queue consumers, DAG execution |
...
- **Auth model:** HS256 JWT — `Authorization: Bearer <token>`. Token blacklist in PostgreSQL `token_blacklist` table. Optional Redis-backed TTL user cache (`ENABLE_USER_CACHE`).
...
| `6.x` | SLO/complexity/timeout/rate limit/idempotency/abuse guard contracts | Enable all middleware guards for production, Redis state sharing | `idempotency_keys` table, slow-query logging | Rate limit env vars, Redis, load tests, SLO alerts |
...
- Datastores: environment-scoped PostgreSQL, Elasticsearch, Redis, and S3. (MongoDB references in older docs are legacy — canonical logs.api storage is S3 CSV; see data ownership table.)
