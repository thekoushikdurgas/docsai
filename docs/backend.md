...
**Language strategy:** The **GraphQL gateway** (`contact360.io/api`) is **Python** (FastAPI + Strawberry). **Satellite backends** standardize on **Go + Gin** for new work and rewrites; exceptions (Django DocsAI, Python jobs stack, legacy `lambda/*`) are listed in **`docs/docs/backend-language-strategy.md`**.

**Docker (EC2):** The root **`docker-compose.prod.yml`** wires internal service hostnames expected by **`contact360.io/api`** (`sync`, `jobs`, `emailapigo`, `s3storage`, `logs`, `salesnavigator`, `contact_ai`, `emailcampaign`, `mailvetter`). Fill **`deploy/env/jobs.env`** for the TKD Job stack and set root **`DATABASE_URL`** for the GraphQL API. See **`deploy/aws/INFRA.md`** for the runbook.

### Orchestration pattern

**Appointment360** (`contact360.io/api`) acts as the **facade / orchestration gateway** for browser and extension clients: it authenticates tenants, applies credits and policy, and fans out work to **satellite HTTP services** (Go/Gin on EC2 for sync, jobs, email, storage, AI, logs, extension, campaign) and legacy Python lambdas where still deployed. Clients should not need to know per-service topology beyond public contracts; internal service discovery uses compose/DNS names in production.
...
| `2.x` | Email module types (finder/verifier), jobs module types (create/list/status), SchedulerJob type | `LambdaEmailClient`, `EmailServerJobsClient` / Connectra jobs, remove debug writes from `email/queries.py` and `jobs/mutations.py` | Activity entries for email jobs; `LAMBDA_EMAIL_*` + `CONNECTRA_*` env vars |
...
| `6.x` | SLO/complexity/timeout/rate limit/idempotency/abuse guard contracts documented | Enable all middleware guards for production; PostgreSQL-backed idempotency | `graphql_idempotency_replays` / idempotency tables; slow-query logging; load tests; SLO alerts |
...
- Data sources span local Django persistence, Appointment360 GraphQL, PostgreSQL-backed gateway state, and S3 index artifacts. (Optional Redis appears only in Go campaign/mailvetter paths; see `docs/docs/data-stores-postgres.md`.)
