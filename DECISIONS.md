...
- **HTTP:** `GET /health`; **`POST /v1/save-profiles`**, **`POST /v1/scrape`**, **`POST /v1/apollo-scrape`** — **`X-API-Key`** / **`api_key`** when **`EXTENSION_API_KEY`** is set — see [`AUTH-ENV.md`](backend/endpoints/extension.server/AUTH-ENV.md). Passthrough routes proxy VQL/bulk to Connectra (e.g. **`POST /contacts/`**, **`POST /companies/`**, **`/contacts/batch-upsert`**, **`/companies/batch-upsert`**). **`POST /v1/apollo-scrape`** parses Apollo.io HTML (same row semantics as [`docs/scripts/apollo_html/main.py`](../docs/scripts/apollo_html/main.py)); body includes **`html`**, optional **`page_url`**, optional **`save: true`** → same Connectra batch-upsert path as **`/v1/scrape`**.
...
- **Browser extension:** uses **`POST /graphql`** on the API gateway (`saveSalesNavigatorProfiles`, `scrapeSalesNavigatorHtml`, `saveApolloProfiles`, `scrapeApolloHtml` with JWT); it does not call extension.server directly. GraphQL **`SaveProfilesResponse`** exposes **`contactUuids`**, **`companyUuids`**, **`savedContacts`**, **`savedCompanies`** (JSON) for the extension preview merge path. The **Apollo** side panel tab uses client-side DOM parsing by default and optional server parse via **`scrapeApolloHtml`** → **`/v1/apollo-scrape`**.
...
*Last updated: 2026-04-20 (Apollo `/v1/apollo-scrape`, GraphQL `apollo.saveApolloProfiles` / `apollo.scrapeApolloHtml`; extension Apollo tab; prior: governance; Connectra batch-upsert UUID contract; extension.server VQL hydrate)*
